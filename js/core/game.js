...
function buildPlayerCombatStatHint(){
  ...
  const parts=[];
  if((s.weaken||0)>0) parts.push(`Weaken ${s.weaken}t (−25% dmg, −40% dodge)`);
...
function refreshBattleUI() {
  ...
  document.getElementById('enemy-stats-mini').innerHTML =
    `${enemyCell('stat-atk','ATK',ep2.atk,{title:'Physical attack'})}
     ...
     ${enemyCell('stat-dodge','Dodge',ep2.dodge||0,{suffix:'%',title:'Dodge'})}
...
function getEffectiveEnemyDodgeForPlayerHit(){
  ...
  if((G.enemyStatus?.weaken||0)>0) d = Math.floor(d * 0.6);
  return Math.max(0, d);
}
...
function dealDamage(target,amount,isCrit=false,isMagic=false,srcAbility=null) {
  ...
  if(isCrit && target==='enemy'){
    ...
    if(_pid==='passive_cassowary_terror_kick' && (es.weaken||0)>0) critDmgAdd+=0.10;
  ...
  if (target==='player') {
    ...
    if(G._playerConfusesSelfThisAction){
      ...
      G.player.stats.hp=Math.max(0,G.player.stats.hp-dmg);
      setHpBar('player',G.player.stats.hp,G.player.stats.maxHp);
      spawnFloat('player',`🌀 -${dmg}`,'fn-dmg');
      logMsg(`🌀 Confused — you hit yourself for ${dmg}!`,'miss');
      return {dmgDealt:dmg,wasDodged:false,wasBlocked:false,isCrit};
    }
  ...
  if(target==='enemy' && G.player){
    ...
    if(pid==='passive_crow_murder_mind' && !G.player._crowMurderMindUsed){
      ...
      if(debKeys.has(ailId) || ailId==='mud'){
        ...
        if(ailId==='weaken') status.weaken=(status.weaken||0)+1;
