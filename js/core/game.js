 ...
 * Build Nest: js/systems/build-nest-state.js, js/systems/build-nest-forge-runtime.js
...
const ABILITY_TEMPLATES = Object.create(null); /* Combat rewrite: populated at boot from equipment skills. */
...
function ensureStatLedger(player){
  ...
  if(!L.fromEquipmentPct || typeof L.fromEquipmentPct !== 'object') L.fromEquipmentPct = {};
  if(!Array.isArray(L.mechanicalLines)) L.mechanicalLines = [];
  return L;
}
function reapplyPlayerGearStats(player){
  if(typeof Avian?.equipment?.reapplyPlayerStatsFromSources==='function'){
    Avian.equipment.reapplyPlayerStatsFromSources(player);
  }
}
globalThis.reapplyPlayerGearStats=reapplyPlayerGearStats;
...
function getBirdAuthoredClassPerk(birdKey){
  ...
  if(!bd) return null;
  if(true){
    const clsV2=typeof Avian.getClassV2==='function'?Avian.getClassV2(bd.class):null;
    if(clsV2?.classPerk){
      return { name: clsV2.classPerk, effect: clsV2.classPerkEffect||bd.classPerkEffect||'' };
    }
  }
...
function getMutationDescHtml(){ return ''; }
function buildMutationTooltipHTML(){ return ''; }
function getEquipmentItem(itemId){
  return typeof Avian?.equipment?.getItem==='function' ? Avian.equipment.getItem(itemId) : null;
}
function getEquipmentSkill(skillId){
...
function formatEquipmentSkillsHtml(item){
  if(!item) return '';
  const rows=[];
  const pushSkill=(id, label)=>{
    if(!id) return;
    const sk=getEquipmentSkill(id);
    rows.push(`<div class="tt-row"><span class="tt-lbl">${escapeHtmlRoster(label)}</span><span class="tt-val" style="font-size:.88em">${escapeHtmlRoster(sk?.name||id)}</span></div>`);
  };
  pushSkill(item.skill1, 'Skill A');
  pushSkill(item.skill2, 'Skill B');
  pushSkill(item.pairedSkill, 'Paired');
  pushSkill(item.ultimate, 'Ultimate');
  return rows.join('');
}
function buildEquipmentTooltipHTML(itemId, opts){
  const item=getEquipmentItem(itemId);
  if(!item) return '';
  opts=opts&&typeof opts==='object'?opts:{};
  const tierMeta=rewardTierMeta(item.rarity||'grey');
  const hands=Number(item.hands)||0;
  const handTxt=item.slot==='Weapon'?(hands===2?'2H':hands===1?'1H':''):'';
  let html=`<div class="tt-name">${escapeHtmlRoster(item.name)}</div>`;
  html+=`<div class="tt-type">${escapeHtmlRoster(tierMeta.label)} · ${escapeHtmlRoster(item.family||item.slot||'Equipment')}${handTxt?` · ${handTxt}`:''}</div>`;
  if(item.classRestriction && item.classRestriction!=='Any'){
    html+=`<div class="tt-row"><span class="tt-lbl">Class</span><span class="tt-val">${escapeHtmlRoster(String(item.classRestriction))}</span></div>`;
  }
  if(item.aspect && item.aspect!=='neutral'){
    html+=`<div class="tt-row"><span class="tt-lbl">Aspect</span><span class="tt-val">${escapeHtmlRoster(formatAspectDisplayName(item.aspect))}</span></div>`;
  }
  const statHtml=formatEquipmentStatsHtml(item);
  if(statHtml) html+=`<div class="tt-mut-stats">${statHtml}</div>`;
  const skillHtml=formatEquipmentSkillsHtml(item);
  if(skillHtml){
    html+=`<div class="tt-type">Skills</div>${skillHtml}`;
  }
  if(Array.isArray(item.bonuses) && item.bonuses.length){
    html+=`<div class="tt-type">Named bonuses</div>`;
    for(const b of item.bonuses){
      const txt=b?.text||b;
      if(txt) html+=`<div class="tt-desc" style="font-size:.88em;margin-bottom:4px">${escapeHtmlRoster(String(txt))}</div>`;
    }
  }
  if(item.uniqueEffect?.text){
    html+=`<div class="tt-type">Unique</div><div class="tt-desc" style="font-size:.88em">${escapeHtmlRoster(item.uniqueEffect.text)}</div>`;
  }
  if(item.tradeoff?.text){
    html+=`<div class="tt-row"><span class="tt-lbl">Tradeoff</span><span class="tt-val" style="font-size:.88em">${escapeHtmlRoster(item.tradeoff.text)}</span></div>`;
  }
  if(!opts.omitClose) html+=richTooltipCloseBtn();
  return html;
}
function buildRichStatTooltipHtml(statKey, rawVal, player){
  if(statKey==='critMult'){
    const base=Number(rawVal)||1.5;
    const eqMech=typeof Avian?.equipment?.getMechanicsRollup==='function'?Avian.equipment.getMechanicsRollup(player):null;
    const bonusPct=(player?.critDamageBonusPct||0)+(eqMech?.critDamageBonusPct||0);
    let html=`<div class="tt-name">CRIT DMG</div>`;
    html+=`<div class="tt-row"><span class="tt-lbl">Base multiplier</span><span class="tt-val">${formatCombatNumber(base)}×</span></div>`;
    if(bonusPct>0) html+=`<div class="tt-row"><span class="tt-lbl">Crit bonus</span><span class="tt-val">+${formatCombatNumber(bonusPct)}</span></div>`;
    html+=`<div class="tt-desc">On critical hits, the bonus is added to the multiplier.</div>`;
    html+=richTooltipCloseBtn();
    return html;
  }
...
function getPlayerEquippedMutationIds(){ return []; }

function buildPlayerBirdTooltipHtml(player){
  if(!player) return '';
  const bk=player.birdKey;
...
function buildClassLabelTooltipHtml(classId, birdKey){
  const role=classToRoleId(classId);
  const label=idToClassLabel(role);
  const flavor=ROLE_FLAVOR[role]||'';
  const perk=getBirdAuthoredClassPerk(birdKey);
  let html=`<div class="tt-name">${escapeHtmlRoster(label)}</div><div class="tt-type">Class</div>`;
  if(flavor) html+=`<div class="tt-desc">${escapeHtmlRoster(flavor)}</div>`;
  if(perk){
    html+=`<div class="tt-row"><span class="tt-lbl">Class perk</span><span class="tt-val" style="font-size:.88em">${escapeHtmlRoster(perk.name)}</span></div>`;
    if(perk.effect) html+=`<div class="tt-desc" style="margin-top:4px;font-size:.88em">${escapeHtmlRoster(perk.effect)}</div>`;
  }
  html+=richTooltipCloseBtn();
  return html;
}
...
function buildEnemyMutationsTooltipHtml(enemy){
  ...
  let html=`<div class="tt-name">${escapeHtmlRoster(name)}</div><div class="tt-type">Equipment loadout</div>`;
  if(enemy.equipment && typeof Avian?.equipment?.getSlotOrder==='function'){
    ...
    for(const sk of order){
      ...
      html+=`<div class="tt-desc" style="opacity:.8;font-size:.78em;margin:0 0 4px">${escapeHtmlRoster(tierMeta.label)} · ${escapeHtmlRoster(item.family||item.slot||'Gear')}</div>`;
      const stats=typeof formatEquipmentCompactStatsHtml==='function'?formatEquipmentCompactStatsHtml(item):'';
      if(stats) html+=`<div class="tt-mut-stats">${stats}</div>`;
      if(item.identity) html+=`<div class="tt-desc" style="font-size:.82em;margin-bottom:4px">${escapeHtmlRoster(item.identity)}</div>`;
    }
    if(!any) html+=`<div class="tt-desc">No equipment equipped.</div>`;
  } else {
    html+=`<div class="tt-desc">No equipment equipped.</div>`;
  }
  html+=`<div class="tt-note" style="opacity:.75;margin-top:6px;font-size:.78em">Click for full Nest details</div>`;
  html+=richTooltipCloseBtn();
  return html;
}
...
function bindHoldRepeat(btn, onTick, opts={}){
  if(!btn || typeof onTick!=='function' || btn._holdRepeatBound) return;
  btn._holdRepeatBound=true;
  let timer=null;
  let startTime=0;
  const interval=opts.interval??120;
  const fastInterval=opts.fastInterval??60;
  const fastAfter=opts.fastAfter??1000;
  const stop=()=>{
  ...
  const schedule=()=>{
    ...
    const delay=elapsed>=fastAfter?fastInterval:interval;
    timer=setTimeout(()=>{
      if(onTick()===false){ stop(); return; }
      schedule();
    }, delay);
  };
  const start=(e)=>{
    if(btn.disabled) return;
    if(e?.type==='touchstart') window._isTouchDevice=true;
    stop();
    btn.classList.add('is-holding');
    startTime=Date.now();
    if(onTick()===false) return;
    schedule();
  };
  btn.addEventListener('mousedown', (e)=>{ if(e.button!==0) return; start(e); });
...
function wireNestEquipmentTooltips(root){
  if(!root) return;
  root.querySelectorAll('[data-nest-eq-item],[data-nest-eq-inv]').forEach(el=>{
...
function wireCombatStatTooltips(container){
  const root=container||document.getElementById('combat-stats-modal-body');
  if(!root) return;
  const grid=root.querySelector('.stats-mini');
  if(!grid) return;
...
function wireCombatClassLabelTooltips(){
  const pcls=document.getElementById('player-class-label');
  if(pcls && G.player){
    bindRichTooltip(pcls, ()=>{
      if(!G.player) return '';
      const cls=resolveFinalClass(G.player.class||BIRDS[G.player.birdKey]?.class||'striker', G.player.birdKey);
      return buildClassLabelTooltipHtml(cls, G.player.birdKey);
    }, { category: 'passives' });
  }
  const ecls=document.getElementById('enemy-class-label');
  if(ecls && G.enemy){
    bindRichTooltip(ecls, ()=>{
      if(!G.enemy) return '';
      const cls=resolveFinalClass(G.enemy.class||inferEnemyClassFromStyle(G.enemy)||'predator', G.enemy.birdKey||'');
      return buildClassLabelTooltipHtml(cls, G.enemy.birdKey||'');
    }, { category: 'passives' });
  }
}
function wireCombatEnemyStatTooltips(container){
  const root=container||document.getElementById('combat-stats-modal-body');
  if(!root) return;
  const grid=root.querySelector('.enemy-stats-mini, .stats-mini');
  if(!grid) return;
  grid.querySelectorAll('[data-stat-key]').forEach(el=>{
    const key = el.dataset.statKey;
    const raw = Number(el.dataset.statRaw);
...
function getDerivedMechanicalBonusLines(player){
  ...
  const mRoll = typeof Avian?.equipment?.getMechanicsRollup==='function' ? Avian.equipment.getMechanicsRollup(p) : null;
  if(mRoll && (mRoll.critDamageBonusPct||0)>0) lines.push(`+${mRoll.critDamageBonusPct}% crit damage (equipment)`);
...
function rollEndlessReward(kind='relic'){
  if(Avian.equipmentLoot && typeof Avian.equipmentLoot.rollEquipmentReward==='function'){
    ...
    return Avian.equipmentLoot.rollEquipmentReward({ rarity: tier, stage: G.stage, isBoss: !!(G.enemy && G.enemy.isBoss), filterForPlayer: true });
  }
  return null;
}
...
function rollUpgradeCard(){
  if(Avian.equipmentLoot && typeof Avian.equipmentLoot.rollEquipmentReward==='function'){
    return Avian.equipmentLoot.rollEquipmentReward({ stage: G.stage, isBoss: !!(G.enemy && G.enemy.isBoss), filterForPlayer: true });
  }
  return null;
}
...
// Category filtering is now derived dynamically from equipment skills
...
function getPlayableStarterBirdKeys(){
  try{
    if(typeof Avian?.data?.motherGooseCatalog?.starterBirdKeys==='function'){
      const keys=Avian.data.motherGooseCatalog.starterBirdKeys();
      if(Array.isArray(keys)&&keys.length) return keys.slice();
    }
  }catch(_){ }
  return ['sparrow','blackbird','macaw','crow','goose'];
}
function collectAllBirdUnlockIds(){
  const ids=new Set();
  Object.values(BIRDS||{}).forEach(bird=>{
    const req=bird&&bird.unlockRequires;
    if(req) ids.add(req);
  });
  return Array.from(ids);
}
function isBirdUnlockedForSelect(key, bird){
  const row=bird||(BIRDS||{})[key];
  if(!row) return false;
  if(getPlayableStarterBirdKeys().includes(key)) return true;
  return !(row.unlockRequires && !isUnlocked(row.unlockRequires));
}
...
function notifyStoryBattleNestEquipLocked(){
  if(true){
    logMsg('Story battle loadouts are locked until victory. Use the Nest from rewards or the overworld to equip gear.','miss');
    return;
  }
  logMsg('Story battle loadouts are locked until victory. Use the Nest from rewards or the overworld to equip gear.','miss');
}
...
function getEquipmentNestSlotLabel(slotKey){
  ...
  return meta?.label||slotKey;
}
function equipmentHandBadgeHtml(item){
  if(!item || item.slot!=='Weapon') return '';
  const hands=Number(item.hands)||0;
  if(hands===2) return '<span class="nest-hand-badge" title="Two-handed">2H</span>';
...
function formatEquipmentStatChipLabel(statKey){
  ...
  const short={
    ...
    matk:'Focus', mdef:'Resolve', spd:'Agi', agility:'Agi', dodge:'Eva', acc:'Prec',
    ...
    armour:'ARM', magicArmour:'MARM',
  };
  if(short[bare]||short[key]) return short[bare]||short[key];
  const full=typeof formatAnyStatLabel==='function'?formatAnyStatLabel(bare||key):key;
...
function formatEquipmentCompactStatsHtml(item){
  ...
  if(item.stats){
    for(const [rawKey, val] of Object.entries(item.stats)){
      ...
      const isPct=String(rawKey).includes('Pct')||/Pct$/i.test(rawKey);
      push(lbl, formatSignedCombatStat(n, isPct));
    }
  }
...
globalThis.setPlayerUltimateSourceItemId=setPlayerUltimateSourceItemId;
if(typeof Avian?.actions?.register==='function'){
  Avian.actions.register('setUltimateSource', function(itemId){
    setPlayerUltimateSourceItemId(itemId);
...
function buildNestEquipmentSectionV2(player){
  ...
  const order=typeof Avian.equipment.getSlotOrder==='function'?Avian.equipment.getSlotOrder():[];
  if(!G._nestActiveEqFilter || (G._nestActiveEqFilter!=='all' && !order.includes(G._nestActiveEqFilter))) G._nestActiveEqFilter='all';
  const activeFilter=G._nestActiveEqFilter;
  const filterHtml=`<div class="nest-slot-filters"><button type="button" class="nest-slot-filter${activeFilter==='all'?' active':''}" data-nest-eq-filter="all">⚙ All</button>${order.map(sk=>{
    const active=sk===activeFilter?' active':'';
    const icon=EQUIPMENT_NEST_SLOT_ICONS[sk]||'⚙';
...
function handleNestEquipmentClick(ev){
  ...
  if(ultPick?.dataset?.nestUltPick){
    setPlayerUltimateSourceItemId(ultPick.dataset.nestUltPick);
    openNest();
    return;
  }
  const el=ev.target.closest('[data-nest-eq-slot],[data-nest-eq-inv],[data-nest-eq-filter]');
  if(!el || !G.player) return;
  if(el.dataset.nestEqFilter){
    G._nestActiveEqFilter=el.dataset.nestEqFilter;
    openNest();
    return;
  }
  ...
  if(el.dataset.nestEqInv){
    const itemId=el.dataset.nestEqInv;
    const slot=typeof Avian?.equipment?.findEquipSlotForItem==='function'?Avian.equipment.findEquipSlotForItem(G.player,itemId):null;
    if(!slot){
      if(typeof logMsg==='function') logMsg('Cannot equip this item — wrong slot or class restriction.', 'system');
      return;
    }
    if(typeof Avian?.equipment?.equip==='function') Avian.equipment.equip(G.player, itemId, slot);
    if(typeof Avian?.equipmentActions?.syncEntityAbilities==='function') Avian.equipmentActions.syncEntityAbilities(G.player);
    saveRun(); openNest(); return;
  }
  const slotKey=el.dataset.nestEqSlot;
  if(el.dataset.nestEqItem && slotKey){
    if(typeof Avian?.equipment?.unequip==='function') Avian.equipment.unequip(G.player, slotKey);
    if(typeof Avian?.equipmentActions?.syncEntityAbilities==='function') Avian.equipmentActions.syncEntityAbilities(G.player);
    saveRun(); openNest(); return;
  }
  if(slotKey){
...
function openNest() {
  ...
  const _eqMech=typeof Avian?.equipment?.getMechanicsRollup==='function'?Avian.equipment.getMechanicsRollup(p):null;
  ...
  const nestUpgRows=[];
  const nestLvlRows=[];
  const _nestBonusChip=(k,v)=>{
    const n=Number(v)||0;
    return `<span class="nest-bonus-chip"><span class="nest-bonus-k">${STAT_LEDGER_LABELS[k]||k}</span><span class="nest-bonus-v">${formatSignedCombatStat(n)}</span></span>`;
  };
  for(const k of STAT_LEDGER_TRACKED_KEYS){
  ...
  if(true) wireNestEquipmentTooltips(content);
  else wireNestMutationTooltips(content);
...
function _nestInventoryMutStatsHtml(){ return ''; }

function _nestMutationItemHtml(){ return ''; }
...
function buildNestEquipmentSection(player){
  if(typeof Avian?.equipment?.ensurePlayerEquipmentState==='function'){
    return buildNestEquipmentSectionV2(player);
  }
  return '';
}
 ...
 * Resolves the tier from the player stub, falling back to the meta card tier so
 * the result is correct regardless of when applyBirdCardProgression last ran.
 */
function getCardTierSlotCount(player){
  const pl = player || G.player;
  let tier = pl?._birdCardTier;
  if(!tier && typeof getBirdCardTier==='function'){
    try{ tier = getBirdCardTier(pl?.birdKey||G.player?.birdKey); }catch(_e){ tier=null; }
  }
  if(typeof getEnemyUnlockedSlotCountForTier==='function'){
    return getEnemyUnlockedSlotCountForTier(tier||'grey');
  }
  return 2;
}
...
const SAVE_BACKUP_KEY_PRE_V13 = globalThis.Avian?.systems?.SAVE_BACKUP_KEY_PRE_V13 ?? 'avianAscent_save_v2_backup_pre_v13';
const EQUIPMENT_V2_MIGRATION_NOTICE_KEY = 'avianAscent_equipmentV2MigrationNotice';
const RUN_SAVE_SCHEMA_VERSION = (typeof globalThis.Avian?.systems?.SAVE_SCHEMA_VERSION === 'number')
  ...
  : 2;
function consumePendingEquipmentV2MigrationCompensation() {
  try {
    const key = globalThis.FORTUNE_META_KEY || 'avianAscent_meta_v1';
    const raw = localStorage.getItem(key);
    if (!raw) return 0;
    const meta = JSON.parse(raw);
    const block = meta?.equipmentV2Migration;
    const pending = Math.max(0, Math.floor(Number(block?.pendingShinyCompensation) || 0));
    if (!pending) return 0;
    if (block) {
      block.pendingShinyCompensation = 0;
      block.lastGrantedAt = Date.now();
      block.lastGrantedAmount = pending;
    }
...
function stashEquipmentV2MigrationNotice(migrationResult) {
  if (!migrationResult) return;
  try {
    sessionStorage.setItem(EQUIPMENT_V2_MIGRATION_NOTICE_KEY, JSON.stringify(migrationResult));
  } catch (_) { /* noop */ }
  G._equipmentV2MigrationNotice = migrationResult;
}
function applyEquipmentLoadoutSanitizationOnLoad(player) {
  if (false || !player) return { issues: [], compensation: 0 };
  if (typeof Avian?.equipment?.ensurePlayerEquipmentState === 'function') {
    Avian.equipment.ensurePlayerEquipmentState(player);
  }
  if (typeof Avian?.equipment?.sanitizeEquipmentLoadout !== 'function') {
    if (typeof Avian?.equipment?.validateLoadout === 'function') {
      return { issues: Avian.equipment.validateLoadout(player), compensation: 0 };
    }
    return { issues: [], compensation: 0 };
  }
  const result = Avian.equipment.sanitizeEquipmentLoadout(player, { removeUnmappable: true });
  if (result.compensation > 0) {
    G.shinyObjects = Math.max(0, Math.floor(Number(G.shinyObjects) || 0)) + result.compensation;
  }
  return result;
}
function finalizeEquipmentV2PreReleaseReset(migrated) {
  if (!migrated?._equipmentV2PreReleaseReset) return false;
  const migrationResult = migrated._equipmentV2MigrationResult || { reset: true };
  stashEquipmentV2MigrationNotice(migrationResult);
  try { localStorage.removeItem(SAVE_KEY); } catch (_) { /* noop */ }
  try {
    localStorage.removeItem(globalThis.AVIAN_OW_KEYS?.STATE ?? 'avianAscent_overworld');
    localStorage.removeItem(globalThis.AVIAN_OW_KEYS?.NAV ?? 'avianAscent_nav');
  } catch (_) { /* noop */ }
  return true;
}
...
function saveRun() {
  ...
  try {
    ...
    if (typeof Avian?.systems?.stampEquipmentSaveFields === 'function') {
      Avian.systems.stampEquipmentSaveFields(save);
    } else if (Avian.flags?.equipmentV2) {
      save.equipmentV2 = true;
      save.equipmentPackVersion = Avian.systems?.EQUIPMENT_PACK_VERSION || '2026.07-equipment-v0.3';
    } else {
      save.equipmentV2 = false;
...
function loadSaveData() {
  try {
    ...
    if(parsed && typeof parsed === 'object'){
      if(typeof Avian?.systems?.maybeBackupPreV13Save === 'function'){
        Avian.systems.maybeBackupPreV13Save(raw, parsed);
      }
      const migrate = globalThis.Avian?.systems?.runSaveMigrations;
      if(typeof migrate === 'function') parsed = migrate(parsed);
      if(!parsed) return null;
      if(parsed._equipmentV2PreReleaseReset){
        finalizeEquipmentV2PreReleaseReset(parsed);
        return null;
      }
    }
    return parsed;
...
function continueRun() {
  ...
  if(true){
    applyEquipmentLoadoutSanitizationOnLoad(G.player);
  }
...
function isDukeEncounterToken(tok){
  const s=String(tok||'').trim();
  const tokNorm=s.toLowerCase().replace(/\s+/g,'');
  const dukeId=typeof getStoryDukeRosterId==='function'?getStoryDukeRosterId():'BO-DUKEB-STORY-L10';
  return tokNorm==='duke_blakiston'||tokNorm==='dukeblakiston'||s===dukeId||s.indexOf('DUKEB')>=0;
}
function rerollNonDukeStageEnemy(encounterStage){
  const pbk=G.player?.birdKey||'sparrow';
  let pool=[];
  if(typeof getStoryStageEnemyCandidateIds==='function') pool=getStoryStageEnemyCandidateIds(encounterStage,pbk).slice();
  else if(typeof pickStoryEncounterEnemyIds==='function') pool=pickStoryEncounterEnemyIds(encounterStage,pbk,1).slice();
  const pick=pool.length?pool[Math.floor(Math.random()*pool.length)]:'EN-SPARR-HESQ-L01';
  return buildOwEnemyDraftFromBirdKey(pick,encounterStage,0);
}
function buildOwEnemyDraftFromBirdKey(bk, encounterStage, slotIdx){
  ...
  if(isDukeEncounterToken(tok) && encounterStage<STORY_DUKE_STAGE){
    return rerollNonDukeStageEnemy(encounterStage);
  }
  if(isDukeEncounterToken(tok) && encounterStage>=STORY_DUKE_STAGE){
...
function resolveEnemyWorkbookLevel(ed, opts={}){
  ...
  if(!opts.isEndless && profile && Number.isFinite(profile.levelOffset)){
...
function applyEnemyStatsFromPlayerProgression(ed, opts={}){
  ...
  /* Endless strength comes from player-relative level and equipment, not battle depth. */
  if(opts.isEndless && rawLevel>30){
    /* Workbook growth ends at 30; continue the selected player-relative level at 4% per excess level. */
...
function mergeScaledStatsIntoEnemy(ed, encounterStage){
  if(!ed) return ed;
  const endless=G.endlessMode && (encounterStage>20 || isEndlessMapActive());
  const diffMult = endless ? 1 : DIFFICULTIES[G.difficulty||'juvenile'].mult;
  const scaleOpts={
    isEndless:endless,
...
function getEnemyKitAbilityIds(enemy){
  ...
  const ids=[];
  for(const x of abs){
    if(x&&typeof x==='object'&&x.empty) continue;
    const id=typeof x==='string'?x:(x&&typeof x==='object'?x.id:'');
    if(id&&id!=='eShield'&&!String(id).startsWith('__empty_')) ids.push(id);
  }
  return ids;
}

function getEnemyEquipmentActionSources(enemy){
  if(!Avian.flags?.equipmentV2||!enemy) return [];
  return (enemy.abilities||[]).filter(a=>a&&!a.empty&&a.id&&!String(a.id).startsWith('__empty_'));
}

function isEnemyUltimateMeterReady(enemy, ab){
  if(!ab?.isUltimate) return true;
  const cfg=Avian?.data?.combatConfig?.ultimateMeter;
  const maxMeter=Number(cfg?.max)||100;
  const meter=Number(enemy?.ultimateMeter);
  if(!Number.isFinite(meter)) return false;
  return meter>=maxMeter;
}

function isEnemyEquipmentActionAvailable(enemy, ab){
  if(!ab||ab.empty||!ab.id||String(ab.id).startsWith('__empty_')) return false;
  if(!isEnemyUltimateMeterReady(enemy, ab)) return false;
  if(typeof getAbilityCooldown==='function' && getAbilityCooldown(ab.id)>0) return false;
  return true;
}
...
function formatEquipmentEffectSummaryHtml(item){
  if(!item) return '';
  const bits=[];
  if(item.identity) bits.push(`<div class="enemy-info-mut-effect">${escapeHtmlRoster(item.identity)}</div>`);
  if(item.uniqueEffect?.text) bits.push(`<div class="enemy-info-mut-effect enemy-info-mut-effect--unique"><strong>Unique:</strong> ${escapeHtmlRoster(item.uniqueEffect.text)}</div>`);
  if(Array.isArray(item.bonuses)){
    for(const b of item.bonuses){
      const txt=b?.text||b;
      if(txt) bits.push(`<div class="enemy-info-mut-effect">${escapeHtmlRoster(String(txt))}</div>`);
    }
  }
  if(item.tradeoff?.text) bits.push(`<div class="enemy-info-mut-effect enemy-info-mut-effect--tradeoff"><strong>Tradeoff:</strong> ${escapeHtmlRoster(item.tradeoff.text)}</div>`);
  if(item.notes && !item.identity) bits.push(`<div class="enemy-info-mut-effect">${escapeHtmlRoster(item.notes)}</div>`);
  return bits.join('');
}

function buildEnemyInfoPopupMutationsHtml(enemy){
  if(!enemy?.equipment) return '<p class="enemy-info-empty">None</p>';
  const slots=(typeof Avian?.equipment?.getSlotOrder==='function'
    ? Avian.equipment.getSlotOrder()
    : (Avian.data?.equipment?.slots?.slotOrder||[]));
  const icons=typeof EQUIPMENT_NEST_SLOT_ICONS!=='undefined'?EQUIPMENT_NEST_SLOT_ICONS:{};
  const rows=slots.map(sk=>{
    const id=enemy.equipment[sk];
    if(!id) return '';
    const item=Avian?.equipment?.getItem?.(id)||(typeof getEquipmentItem==='function'?getEquipmentItem(id):null);
    if(!item) return '';
    const slotLbl=typeof getEquipmentNestSlotLabel==='function'?getEquipmentNestSlotLabel(sk):sk;
    const icon=icons[sk]||equipmentSlotIconForItem?.(item)||'⚙';
    const tierMeta=rewardTierMeta(item.rarity||'grey');
    const tierColor=typeof nestTierColorVar==='function'?nestTierColorVar(item.rarity):'var(--silver)';
    const handBadge=typeof equipmentHandBadgeHtml==='function'?equipmentHandBadgeHtml(item):'';
    const statsBlock=typeof formatEquipmentCompactStatsHtml==='function'?formatEquipmentCompactStatsHtml(item):'';
    const statsHtml=statsBlock?`<div class="enemy-info-mut-stats nest-mut-stats mut-stat-compact-wrap">${statsBlock}</div>`:'';
    const skillsHtml=typeof formatEquipmentSkillsHtml==='function'?formatEquipmentSkillsHtml(item):'';
    const skillsBlock=skillsHtml?`<div class="enemy-info-mut-skills">${skillsHtml}</div>`:'';
    const effectHtml=formatEquipmentEffectSummaryHtml(item);
    const classTag=item.classRestriction&&item.classRestriction!=='Any'
      ?`<span class="enemy-info-mut-class">${escapeHtmlRoster(String(item.classRestriction))}</span>`:'';
    return `<div class="enemy-info-mut-row enemy-info-eq-row" data-enemy-eq-item="${escapeHtmlRoster(String(id))}" tabindex="0" role="button" aria-label="${escapeHtmlRoster(item.name)} equipment details">
<span class="enemy-info-mut-icon" aria-hidden="true">${icon}</span>
<div class="enemy-info-mut-body">
<div class="enemy-info-mut-slot">${escapeHtmlRoster(slotLbl)}${handBadge}${classTag}</div>
<div class="enemy-info-mut-tier" style="color:${tierColor}">${escapeHtmlRoster(tierMeta.label)} · ${escapeHtmlRoster(item.family||item.slot||'Gear')}</div>
<div class="enemy-info-mut-name" style="color:${tierColor}">${escapeHtmlRoster(item.name)}</div>
${statsHtml}${skillsBlock}${effectHtml}
</div></div>`;
  }).filter(Boolean);
  return rows.length?rows.join(''):'<p class="enemy-info-empty">None</p>';
}

function wireEnemyInfoEquipmentTooltips(root){
  if(!root || typeof bindRichTooltip!=='function') return;
  root.querySelectorAll('[data-enemy-eq-item]').forEach(el=>{
...
function buildEncounterPreviewTooltipHtml(enemy){
  ...
  const mutBlock=buildEncounterPreviewEquipmentHtml(enemy);
...
function buildEncounterPreviewEquipmentHtml(enemy){
  ...
  for(const sk of order){
    const id=eq[sk];
    if(!id) continue;
    const item=Avian.equipment.getItem(id);
    if(!item) continue;
    const slotLbl=typeof getEquipmentNestSlotLabel==='function'?getEquipmentNestSlotLabel(sk):sk;
    const stats=typeof formatEquipmentCompactStatsHtml==='function'?formatEquipmentCompactStatsHtml(item):'';
    lines.push(`<li><strong>${escapeEncounterPreviewHtml(slotLbl)}:</strong> ${escapeEncounterPreviewHtml(item.name)}${stats?`<div class="enc-preview-mut-stats mut-stat-compact-wrap">${stats}</div>`:''}${item.identity?`<div style="font-size:.65rem;color:var(--text-dim)">${escapeEncounterPreviewHtml(item.identity)}</div>`:''}</li>`);
  }
  if(!lines.length) return '';
  return `<div class="enc-preview-tt-skills" style="margin-top:8px"><div class="enc-preview-tt-skills-h">Equipment</div><ul class="enc-preview-tt-ul">${lines.join('')}</ul></div>`;
}
...
globalThis.STORY_BOSS_STAT_MULT = STORY_BOSS_STAT_MULT;
...
function buildStoryEnemyFromBirdKey(birdKey, stage, opts={}){
  ...
  const diffMult = opts.isEndless ? 1 : (DIFFICULTIES[G.difficulty||'juvenile']?.mult || 1);
...
function buildGameModeToggle(){
  ...
  host.innerHTML=`<div class="flight-choice-grid flight-choice-grid--mode" role="radiogroup" aria-labelledby="flight-mode-heading">` +
    `<button type="button" class="flight-choice-card${!isEndless?' is-active':''}" role="radio" aria-checked="${!isEndless?'true':'false'}" data-action="setGameMode:story">` +
    `<span class="flight-choice-emoji" aria-hidden="true">📖</span>` +
    `<span class="flight-choice-copy"><span class="flight-choice-title">Story Mode</span>` +
    `<span class="flight-choice-desc">Follow Blakiston's Court — stages, bosses, and the campaign ending.</span></span></button>` +
    `<button type="button" class="flight-choice-card${isEndless?' is-active':''}" role="radio" aria-checked="${isEndless?'true':'false'}" data-action="setGameMode:endless">` +
    `<span class="flight-choice-emoji" aria-hidden="true">♾</span>` +
    `<span class="flight-choice-copy"><span class="flight-choice-title">Endless Mode</span>` +
    `<span class="flight-choice-desc">Keep climbing after the court. Enemies scale as you go.</span></span></button>` +
    `</div>`;
  if(typeof syncFlightSettingsBriefing==='function') syncFlightSettingsBriefing();
}
...
function renderStarterFallbackGrid(reason=''){
  ...
  if(!grid) return;
  grid.innerHTML = '';
  const starters = (typeof getPlayableStarterBirdKeys==='function')
    ? getPlayableStarterBirdKeys()
    : ['sparrow','goose','blackbird','crow','macaw'];
  ...
  if(label) label.textContent=`${starters.length}/${starters.length} birds unlocked (recovery)`;
...
function formatDifficultyMult(mult){
  const n=Number(mult);
...
function buildDifficultyPicker() {
  ...
  Object.values(DIFFICULTIES).forEach(d => {
    ...
    const selected = G._selectedDifficulty===d.id;
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'flight-choice-card flight-diff-card' + (locked?' is-locked locked-diff':'') + (selected?' is-active':'');
    btn.setAttribute('role','radio');
    btn.setAttribute('aria-checked', selected && !locked ? 'true' : 'false');
    btn.style.setProperty('--flight-diff-color', d.color || '#e8c96a');
    const lockHint = 'Clear Predator to unlock Murder';
    const descHtml = escapeHtmlRoster(d.desc || '');
    const titleHtml = escapeHtmlRoster(d.label || d.id);
    const emojiHtml = escapeHtmlRoster(d.emoji || '');
    const multHtml = escapeHtmlRoster(formatDifficultyMult(d.mult));
    btn.innerHTML =
      `<span class="flight-choice-emoji" aria-hidden="true">${emojiHtml}</span>` +
      `<span class="flight-choice-copy">` +
      `<span class="flight-diff-topline"><span class="flight-choice-title">${titleHtml}</span>` +
      (multHtml?`<span class="flight-diff-mult">${multHtml}</span>`:'') +
      `</span>` +
      `<span class="flight-choice-desc">${descHtml}</span>` +
      (locked?`<span class="flight-diff-lock">🔒 ${escapeHtmlRoster(lockHint)}</span>`:'') +
      `</span>`;
    if(locked){
      btn.disabled = true;
      ...
      btn.title = d.scalingTip || d.desc || '';
      btn.dataset.action = 'selectDifficulty:' + d.id;
    }
    container.appendChild(btn);
  });
  // Set default
  if(!G._selectedDifficulty) { G._selectedDifficulty='juvenile'; buildDifficultyPicker(); return; }
  const desc = document.getElementById('diff-desc');
  const cur = DIFFICULTIES[G._selectedDifficulty||'juvenile'];
  if(desc) desc.textContent = cur ? `${cur.emoji} ${cur.label} — ${cur.scalingTip || cur.desc}` : 'Choose your difficulty to begin.';
  if(typeof syncFlightSettingsBriefing==='function') syncFlightSettingsBriefing();
}
...
function buildBirdGrid() {
  ...
  const fallbackStarters = (typeof getPlayableStarterBirdKeys==='function')
    ? getPlayableStarterBirdKeys()
    : ['sparrow','goose','blackbird','crow','macaw'];
  ...
  if(totalBirds===0){
    ...
    section.appendChild(row);
    grid.appendChild(section);
    if(label) label.textContent=`${fallbackStarters.length}/${fallbackStarters.length} birds unlocked (fallback)`;
  }
}
...
function getFlightSettingsSummary(){
  const ui=ensureUIState();
  ...
  return {diffLabel, modeLabel, mapLabel, mapVariant, diffId};
}

function syncFlightSettingsBriefing(){
  const wrap=document.getElementById('flight-briefing');
  if(!wrap) return;
  const s=getFlightSettingsSummary();
  wrap.innerHTML=
    `<span class="flight-briefing-kicker">This flight</span>` +
    `<span class="flight-briefing-chips">` +
    `<span class="flight-briefing-chip flight-briefing-chip--map">${escapeHtmlRoster(s.mapLabel)}</span>` +
    `<span class="flight-briefing-chip">${escapeHtmlRoster(s.diffLabel)}</span>` +
    `<span class="flight-briefing-chip flight-briefing-chip--mode">${escapeHtmlRoster(s.modeLabel)}</span>` +
    `</span>`;
}
globalThis.syncFlightSettingsBriefing=syncFlightSettingsBriefing;
globalThis.getFlightSettingsSummary=getFlightSettingsSummary;

function syncSfselRunSummary(){
  const wrap=document.getElementById('sfsel-run-summary');
  const s=getFlightSettingsSummary();
  if(wrap){
    wrap.innerHTML=`<span class="sfsel__run-chip">${escapeHtmlRoster(s.diffLabel)}</span><span class="sfsel__run-chip sfsel__run-chip--mode">${escapeHtmlRoster(s.modeLabel)}</span><span class="sfsel__run-chip sfsel__run-chip--map">${escapeHtmlRoster(s.mapLabel)}</span>`;
  }
  syncFlightSettingsBriefing();
}
...
function buildBirdUpgradePreviewModel(birdKey){
  ...
  const passiveText=passiveInfo?.desc||passiveInfo?.effect||bird.passive?.desc||'';
  ...
  return{
    ...
    passiveName,
    passiveBefore:passiveText,
    passiveAfter:passiveText,
...
function birdUpgradeStatRowsHtml(model){
  return model.statRows.map(row=>{
    ...
    const growthNote=row.growthLabel?`<small class="bird-upgrade-growth-tag">${escapeHtmlRoster(row.growthLabel)}</small>`:'';
    return `<div class="bird-upgrade-stat-row"><span>${escapeHtmlRoster(row.label)}${growthNote}</span><strong>${escapeHtmlRoster(formatBirdUpgradeStatValue(row.before))}</strong><i aria-hidden="true">→</i><strong>${escapeHtmlRoster(formatBirdUpgradeStatValue(row.after))}</strong><em class="${deltaClass}">${escapeHtmlRoster(deltaText)}</em></div>`;
  }).join('');
}
...
function renderBirdUpgradePreviewModal(model, state='preview'){
  ...
  const resultBanner=isResult
    ? `<div class="bird-upgrade-result-banner">${model.isTierUp?'Tier advanced — new ability may unlock.':'Upgrade complete.'}</div>`
  ...
  content.innerHTML=`
    ...
    <div class="bird-upgrade-passive">
      <h4>${escapeHtmlRoster(model.passiveName)}</h4>
      <p>${escapeHtmlRoster(model.passiveBefore||'No passive listed.')}</p>
      <p class="bird-upgrade-passive-note">Passives and class perks do not change with stars or tiers.</p>
    </div>
...
function updateAscentPanel(key) {
  ...
  try{
    ...
    const statRow=(label,value)=>`<dt>${label}</dt><dd>${value}</dd>`;
    const statGroup=(heading,rows)=>`
      <section class="ascent-stat-group">
        <h4>${heading}</h4>
        <dl>${rows.join('')}</dl>
      </section>`;
    const statsStrip=`
      <div class="ascent-stats-strip">
        ${statGroup('Core',[
          statRow(ledgerStatLabel('maxHp'),dispStats.maxHp??dispStats.hp),
          statRow(ledgerStatLabel('vitality'),Number(dispStats.vitality)||0),
          statRow('Energy',`${startEnShow}/${maxEn}`),
        ])}
        ${statGroup('Offense',[
          statRow(ledgerStatLabel('atk'),dispStats.atk),
          statRow(ledgerStatLabel('dex'),Number(dispStats.dex)||0),
          statRow(ledgerStatLabel('acc'),Number(dispStats.acc)||0),
          statRow(ledgerStatLabel('matk'),Number(dispStats.matk)||0),
          statRow(ledgerStatLabel('critChance'),`${cc}%`),
          statRow(ledgerStatLabel('critMult'),`${cd.toFixed(1)}×`),
        ])}
        ${statGroup('Defense & mobility',[
          statRow(ledgerStatLabel('def'),dispStats.def),
          statRow(ledgerStatLabel('mdef'),Number(dispStats.mdef)||0),
          statRow(ledgerStatLabel('spd'),dispStats.spd),
          statRow(ledgerStatLabel('dodge'),`${Number(dispStats.dodge)||0}%`),
        ])}
      </div>`;
    ...
    panel.innerHTML = `
      <article class="ascent-strip ascent-strip--filled ascent-strip--${tierCss} sfsel-profile">
        <header class="sfsel-profile__identity">
          <div class="sfsel-profile__portrait" aria-hidden="true"><div class="ascent-panel-portrait">${renderBirdIconHTML(key, sizeClass, false)}</div></div>
          <div class="sfsel-profile__identity-copy">
            <div class="sfsel-profile__title">
              ...
              <span class="bird-card-tier-badge ${tierCss}">${tierLabel}</span>
              <span class="ascent-card-stars-wrap" aria-label="${cardStars} of ${maxStars} stars">${starsHtml}</span>
            </div>
            <div class="ascent-strip-tagline">${escapeHtmlRoster(bird.tagline||'')}</div>
            ${cardHint}
            <div class="sfsel-profile__progression" aria-label="Species progression">
        ...
        <div class="sfsel-profile__details">
          <section class="sfsel-profile__region sfsel-profile__stats" aria-labelledby="sfsel-stats-heading">
              <h3 class="sfsel-profile__heading" id="sfsel-stats-heading">Stats</h3>
              ${statsStrip}
          </section>
          <section class="sfsel-profile__region sfsel-profile__traits" aria-labelledby="sfsel-traits-heading">
...
function startGame() {
  ...
  G.player = {
    ...
    poisonCap: 5,
    endlessRewards: [],
    critChance: baseStats.critChance||bd.stats.critChance||5,
    passive: bd.passive||null,
    innateUtilityId: bd.innateUtilityId||(Avian.flags?.equipmentV2&&typeof Avian.getInnateUtility==='function'&&Avian.getInnateUtility(G.selected)?G.selected:null),
    innateUtilityName: bd.innateUtilityName||(Avian.getInnateUtility?.(G.selected)?.name)||null,
    cardDodge: 0,
    energyMax: 0,
    energy: 0,
    energyRegen: 0,
  ...
  G.shinyObjects = consumePendingEquipmentV2MigrationCompensation();
  G.runClassPerks = [];
  G.classPerks = {};
...
function grantEndlessMapTreasure(){
  ...
  if(G.endlessMap){
    G.endlessMap.lastMessage = 'Treasure: +' + shinyGain + ' 🌟' + gearLabel + '.';
  }
}
...
function setSuppliesSubView(which){
  ...
  const subnav=document.querySelector('#select-hub-supplies .supplies-subnav');
  if(subnav) subnav.classList.toggle('hub-subnav--3', !!(bCodes && !bCodes.hidden));
  if(isRec){
    try{
...
function syncMissionMapVariantTabs(which){
  ...
  if(bDemo){
    ...
    bDemo.setAttribute('aria-checked', String(!isTest));
  }
  if(bTest){
    bTest.classList.toggle('is-active', isTest);
    bTest.setAttribute('aria-selected', String(isTest));
    bTest.setAttribute('aria-checked', String(isTest));
  }
...
function openSelectHubPanel(which){
  ...
  if(which === 'map'){
    ...
    refreshMissionTestMapSelect();
    if(typeof buildDifficultyPicker==='function') buildDifficultyPicker();
...
function showScreen(id) {
  ...
  if(id==='screen-stork-shop' || id==='screen-grove' || id==='screen-map-forge' || id==='screen-reward' || id==='screen-gameover'){
...
function getEffectiveAbilityBtnType(ab, tmpl){
  if(ab?._dispatcherRow && typeof globalThis.resolveCombatRowBtnType==='function'){
    return globalThis.resolveCombatRowBtnType(ab._dispatcherRow);
  }
...
function combatFxApi(){
  try{ return (Avian && Avian.ui && Avian.ui.combatFx) || null; }catch(_){ return null; }
}
...
async function doAttack(attacker,target,result) {
  ...
  if(fx && typeof fx.prepareCombatStrike==='function'){
    try{ fx.prepareCombatStrike(attacker, target, result, kind); }catch(_){}
  }
...
async function doSpell(target,text) {
  ...
  const fx=combatFxApi();
  if(fx && typeof fx.prepareCombatCast==='function'){
    try{ fx.prepareCombatCast(caster, target, text); }catch(_){}
  }
...
async function doHeal(who,amt) {
  const fx=combatFxApi();
  if(fx && typeof fx.onHeal==='function'){
    try{ fx.onHeal(who, amt); }catch(_){}
  }
  ...
  if(who==='player'&&G.player){
    ...
    if(typeof Avian?.equipmentEffects?.onHeal==='function') Avian.equipmentEffects.onHeal('player', amt);
    setHpBar('player',G.player.stats.hp,G.player.stats.maxHp);
  }
  await delay(400);
}
...
function getPlayerAccMod(){
  ...
  mod+=(G.playerStatus?.passiveAcc||0);
  return mod;
}
...
function getPhysicalPierceFractionForDamage(ab){
  ...
  const wbDefPen = (Number(G._workbookPassiveDefPen) || 0) / 100;
  const total = fromAb + pts / 100 + eqPct / 100 + wbDefPen;
  if(Avian.combat?.clampPen) return Avian.combat.clampPen(total);
...
function getMagicalPierceFractionForDamage(ab){
  ...
  const total = fromAb + pts / 100 + eqPct / 100 + classPen;
  if(Avian.combat?.clampPen) return Avian.combat.clampPen(total);
  return Math.min(0.95, total);
}
...
// - Endless reuses Story's stat-growth tables, but combat depth adds no power.
// - Enemy level is derived from the player's current level plus the difficulty offset.
// - Levels beyond the workbook cap continue through the player-relative overflow rule.
...
function getEndlessDifficultyLevelOffset(difficulty, enemy){
  const diff=String(difficulty||'juvenile').toLowerCase();
  if(diff==='fletchling') return -1;
  if(diff==='predator') return 1;
  if(diff==='murder'){
    const key=String(enemy?.id||enemy?.rosterId||enemy?.birdKey||enemy?.name||'enemy');
    let hash=0;
...
function pickUniqueMutationReward(tier, used, isBoss){
  if(typeof Avian?.equipmentLoot?.rollEquipmentReward!=='function') return null;
...
function buildScaledEnemy(enemyBase, stage, opts={}){
  ...
  acc=Math.max(60,Math.min(96,Math.floor(acc*diffMult*storyMult)));
  dodge=Math.max(0,Math.min(42,Math.floor(dodge*diffMult*storyMult)));
...
function applyGuardedBuff(side, opts={}){
  ...
  const sourceKind=opts.sourceKind?String(opts.sourceKind):'ability';
  if(side==='player' && sourceAbilityId!=='mutation_shield' && typeof Avian?.equipment?.getMechanicsRollup==='function'){
    const mech=Avian.equipment.getMechanicsRollup(G.player);
  ...
  if(cur&&typeof cur==='object'){
    status.guarded={
      ...
      sourceKind:sourceKind||cur.sourceKind||'ability',
    };
  }else{
    status.guarded={turns, physReducPct:pct, sourceAbilityId, sourceKind};
  }
}
...
function applyDamageThroughShield(stats, status, dmg, isMagic=false, opts=null){
  ...
  stats.shieldHp=Math.round((shield-absorbed)*100)/100;
  remaining=Math.round((remaining-absorbed)*100)/100;
  if(stats.shieldHp<=0){
    stats.shieldHp=0;
    stats.maxShieldHp=0;
    if(status){
      delete status.shieldHpTurns;
      delete status.shieldHpSourceId;
      delete status.shieldHpSourceKind;
    }
  }
  return remaining;
}
...
function scaleHealForBleed(who, raw){
  ...
  if(who==='player' && typeof Avian?.equipmentEffects?.getHealingReceivedMultiplier==='function'){
    scaled *= Avian.equipmentEffects.getHealingReceivedMultiplier();
  }
  if(who!=='player' && typeof Avian?.equipmentEffects?.getHealingDoneMultiplier==='function'){
    scaled *= Avian.equipmentEffects.getHealingDoneMultiplier();
  }
...
function detectEquipmentDamageBonus(player){
  if(!player||typeof Avian?.equipment?.getMechanicsRollup!=='function') return false;
  const eqM=Avian.equipment.getMechanicsRollup(player);
  if(eqM.damageBonuses&&eqM.damageBonuses.length) return true;
  if((eqM.physicalDamageUpPct||0)>0||(eqM.magicDamageUpPct||0)>0) return true;
  return false;
}
...
function computeMasterOutgoingDamage(isMagic, srcAbility, opts={}){
  ...
  const battleState={
    ...
    hasMutationEquipmentBonus:detectEquipmentDamageBonus(G.player),
  };
...
function collectOutgoingDamageBonusFractions(ctx){
  ...
  if(p&&typeof Avian?.equipment?.getMechanicsRollup==='function'){
    const _eqM=Avian.equipment.getMechanicsRollup(p);
  ...
  if(typeof Avian?.equipmentEffects?.getOutgoingDamageBonusFractions==='function'){
    const _attackWeight=activeAb?getAbilityAttackWeight(activeAb,p):null;
    const _mf=Avian.equipmentEffects.getOutgoingDamageBonusFractions({
...
function applyLifestealFromDamage(dmg, srcAbility){
  ...
  if(typeof Avian?.equipmentEffects?.getLifestealPct==='function') pct+=Avian.equipmentEffects.getLifestealPct();
  if(pct<=0) return;
...
function applyCurvedMitigationToPlayer(preMit,isMagic,srcAbility){
  ...
  if(typeof Avian?.classPerks?.getIncomingDamageMultiplier==='function'){
    const _bulwarkMult=Avian.classPerks.getIncomingDamageMultiplier();
    if(_bulwarkMult<1 && typeof Avian?.classPerks?.markBulwarkOathConsumed==='function') Avian.classPerks.markBulwarkOathConsumed(G.player);
    mitigated*=_bulwarkMult;
  }
...
function computePlayerCritDamageAdd(activeAb){
  ...
  if(typeof Avian?.equipment?.getMechanicsRollup==='function'){
    const _eqCritM=Avian.equipment.getMechanicsRollup(G.player);
...
function dealDamage(target,amount,isCrit=false,isMagic=false,srcAbility=null,opts=null) {
  ...
  if(target==='enemy'&&!usedMasterPath){
    ...
    if(isCrit){
      ...
      if(typeof Avian?.equipmentEffects?.onPlayerCrit==='function') Avian.equipmentEffects.onPlayerCrit();
    }
    if(typeof globalThis.avianApplySynergyConsumeDamage==='function'){
      dmg=globalThis.avianApplySynergyConsumeDamage('enemy',isCrit,isMagic,dmg);
    }
...
function getDelayedDmgBoostPct() {
  ...
  const eqM = (typeof Avian !== 'undefined' && Avian.equipment && typeof Avian.equipment.getMechanicsRollup === 'function')
    ? Avian.equipment.getMechanicsRollup(G.player) : null;
...
async function playerAction(ab,fromQueue=false) {
  ...
  if(ab?.empty||ab?.disabled){logMsg(ab?.reason||'That action slot is empty.','miss');return;}
  ...
  if(_defSkill){
    ...
    if(typeof Avian?.equipmentEffects?.onPlayerGuard==='function') Avian.equipmentEffects.onPlayerGuard();
  }
  ...
  if((effActKind==='utility'||effActKind==='buff'||effActKind==='defend'||effActKind==='heal')){
    const fx=combatFxApi();
    if(fx && typeof fx.prepareCombatUtility==='function'){
      try{ fx.prepareCombatUtility('player', ab); }catch(_){}
    }
  }
  ...
  if(typeof computeUltimateMeterAward==='function' && typeof awardUltimateMeter==='function'){
    ...
    if(typeof Avian?.equipmentEffects?.getUltimateMeterMultiplier==='function'){
      meterGain=Math.round(meterGain * Avian.equipmentEffects.getUltimateMeterMultiplier());
    }
...
function startPlayerTurn(player){
  if(typeof Avian?.equipmentEffects?.onPlayerTurnStart==='function') Avian.equipmentEffects.onPlayerTurnStart();
...
function projectedEnemyActionDamage(a,e){
  ...
  if(btn==='physical'||btn==='ranged'||btn==='spell'){
    ...
    if(mult==null && ab._dispatcherRow?.abilityPower!=null){
      mult=Math.max(0.25, Number(ab._dispatcherRow.abilityPower)/100);
    }
...
function renderLuFeatherIcons(spent, total){
  const wrap=document.getElementById('lu-feather-icons');
  if(!wrap) return;
  const n=Math.max(0, Math.floor(Number(total)||0));
  if(n<=0){ wrap.innerHTML=''; return; }
  wrap.innerHTML=Array.from({length:n}, (_,i)=>`<span class="lu-feather-icon${i<spent?' is-spent':''}" title="Feather ${i+1}">🪶</span>`).join('');
}

function renderLuFeatherStatline(opt){
  const statKey=luStatKeyForOption(opt);
  const before=G._luStatBaseline||captureLuStatBaseline();
  const after=simulateLuDraftStats();
  const isPct=statKey==='dodge';
  const fmt=(v)=>isPct?`${v}%`:String(v);
  const cur=getLuFeatherStatValue(before, statKey);
  const proj=getLuFeatherStatValue(after, statKey);
  const label=ledgerStatLabel(statKey)||statKey;
  const perPoint=getLevelUpPerPointBreakdown(statKey, 1)[0]||'';
  const hint=perPoint?`<span class="lu-feather-hint">${escapeHtmlRoster(perPoint)}</span>`:'';
  if(Math.abs(proj-cur)<0.0001){
    return `<span class="lu-feather-current">${escapeHtmlRoster(label)} <strong>${escapeHtmlRoster(fmt(cur))}</strong></span>${hint}`;
  }
  return `<span class="lu-feather-current">${escapeHtmlRoster(label)} ${escapeHtmlRoster(fmt(cur))}</span><span class="lu-feather-arrow">→</span><span class="lu-feather-projected">${escapeHtmlRoster(fmt(proj))}</span>${hint}`;
}
...
function refreshLuFeatherPanelUI(){
  ...
  const unalloc=luFeathersUnallocated();
  const spent=luFeatherDraftTotal();
  const total=Math.max(0, Number(G._pendingLevelUpChoices)||0);
  if(rem) rem.innerHTML=`Remaining Feathers: <strong>${unalloc}</strong>${total?` <span style="opacity:.75">(${spent}/${total} spent)</span>`:''}`;
  renderLuFeatherIcons(spent, total);
  const resetBtn=document.getElementById('lu-reset-btn');
  if(resetBtn) resetBtn.disabled=spent<=0;
  const opts=G._luFeatherPanelOptions;
  if(Array.isArray(opts)){
    for(const opt of opts){
      ...
      const badgeEl=document.getElementById(`lu-fb-${opt.id}`);
      if(badgeEl) badgeEl.hidden=n<=0;
      if(badgeEl && n>0) badgeEl.textContent=`×${n}`;
      const statline=document.getElementById(`lu-fs-${opt.id}`);
      if(statline) statline.innerHTML=renderLuFeatherStatline(opt);
      ...
      if(plus) plus.disabled=unalloc<=0;
  ...
  if(btn){
    const ok=unalloc===0 && spent>0;
    btn.className=ok?'confirm-btn visible':'confirm-btn';
    btn.disabled=!ok;
    btn.textContent=ok?`✓ Confirm ${spent} Upgrade${spent===1?'':'s'}`:'✓ Confirm Stat Upgrade';
  }
}
...
function buildFeatherStatPanel(){
  ...
  for(const opt of G._luFeatherPanelOptions){
    const isRare=String(opt.id||'').includes('_rare')||String(opt.id||'').endsWith('r');
    const row=document.createElement('div');
    row.className='lu-feather-row'+(isRare?' is-rare':'');
    row.id=`lu-fr-${opt.id}`;
    row.dataset.statKey=luStatKeyForOption(opt);
    row.innerHTML=`
      <div class="lu-feather-row-top">
        <div class="lu-feather-info">
          <span class="lu-feather-name">${opt.label}${isRare?' ★':''}</span>
        ...
        <div class="lu-feather-stepper">
          ...
          <span class="lu-feather-badge" id="lu-fb-${opt.id}" hidden></span>
      ...
      <div class="lu-feather-row-bottom">
        <div class="lu-feather-statline" id="lu-fs-${opt.id}"></div>
      </div>`;
    const minus=row.querySelector(`#lu-fp-minus-${opt.id}`);
    const plus=row.querySelector(`#lu-fp-plus-${opt.id}`);
    const dec=()=>{
    ...
    const inc=()=>{
      if(luFeathersUnallocated()<=0) return false;
      const cur=Math.max(0,Math.floor(Number((G._luFeatherDraft||{})[opt.id])||0));
      G._luFeatherDraft[opt.id]=cur+1;
      refreshLuFeatherPanelUI();
      return true;
    };
    bindHoldRepeat(minus, ()=>{ dec(); return true; });
    bindHoldRepeat(plus, ()=>{ inc(); return true; });
    grid.appendChild(row);
  }
  refreshLuFeatherPanelUI();
}
...
function getEquippedWeaponAvgDamage(){
  const p=G.player;
  if(!p) return 2.5;
  let item=null;
  if(typeof Avian?.equipment?.getEquippedItems==='function'){
    const eq=Avian.equipment.getEquippedItems(p);
    const raw=eq?.mainHand||eq?.mainWeapon||eq?.weapon;
    if(raw && typeof raw==='object') item=raw;
    else if(raw && typeof Avian?.equipment?.getItem==='function') item=Avian.equipment.getItem(raw);
  }
  if(!item && p.equipment?.mainHand && typeof Avian?.equipment?.getItem==='function'){
    item=Avian.equipment.getItem(p.equipment.mainHand);
  }
  if(item && (item.minDamage!=null||item.maxDamage!=null)){
    const lo=Number(item.minDamage??item.maxDamage)||0;
    const hi=Number(item.maxDamage??item.minDamage)||lo;
    return (lo+hi)/2;
  }
  return 2.5;
}
...
function getOffencePctPerStat(){
  const v=Avian?.data?.combatConfig?.weaponFirst?.offencePctPerStat;
  return v!=null?Number(v):2.5;
}

function estimateMitigationPct(defValue){
  const getMit=Avian?.combat?.getMitigationFraction||globalThis.getMitigationFraction;
  if(typeof getMit!=='function') return null;
  return Math.round(getMit(Math.max(0, Number(defValue)||0))*1000)/10;
}

function estimateWeaponSkillDamagePerStat(statKey, points=1){
  const wd=getEquippedWeaponAvgDamage();
  const gain=wd*getOffencePctPerStat()*Math.max(0, Number(points)||0)/100;
  return Math.round(gain*100)/100;
}

function getLevelUpPerPointBreakdown(statKey, points=1){
  const n=Math.max(1, Math.floor(Number(points)||1));
  const label=ledgerStatLabel(statKey)||statKey;
  const lines=[];
  const cur=Number(G.player?.stats?.[statKey])||0;
  if(statKey==='vitality'){
    const per=(typeof Avian?.birdProgression?.vitalityMaxHpPerPoint==='function')
      ? Avian.birdProgression.vitalityMaxHpPerPoint() : 3;
    const hpGain=n*per;
    lines.push(`+${hpGain} Max Health`);
    lines.push(`${per} HP per Vitality point`);
    return lines;
  }
  if(statKey==='maxHp'){
    lines.push(`+${n} Max Health directly`);
    return lines;
  }
  if(statKey==='atk'){
    const dmg=estimateWeaponSkillDamagePerStat('atk', n);
    lines.push(`+${formatCombatNumber(dmg)} damage on 100% Martial weapon skills`);
    lines.push(`Scales Strength/Martial abilities`);
    return lines;
  }
  if(statKey==='matk'){
    const dmg=estimateWeaponSkillDamagePerStat('matk', n);
    lines.push(`+${formatCombatNumber(dmg)} damage on 100% Magic weapon skills`);
    lines.push(`Scales Focus/Magic abilities`);
    return lines;
  }
  if(statKey==='dex'){
    const dmg=estimateWeaponSkillDamagePerStat('dex', n);
    lines.push(`+${formatCombatNumber(dmg)} damage on 100% Finesse weapon skills`);
    lines.push(`Improves Precision-based skill scaling`);
    return lines;
  }
  if(statKey==='def'){
    const before=estimateMitigationPct(cur);
    const after=estimateMitigationPct(cur+n);
    if(before!=null && after!=null){
      lines.push(`${after}% Martial damage resisted (was ${before}%)`);
      lines.push(`+${formatCombatNumber(after-before)}% resist per +${n} ${label}`);
    }else{
      lines.push(`+${n} ${label} reduces Martial damage taken`);
    }
    return lines;
  }
  if(statKey==='mdef'){
    ...
    const after=estimateMitigationPct(cur+n);
    if(before!=null && after!=null){
      lines.push(`${after}% Magic damage resisted (was ${before}%)`);
      lines.push(`+${formatCombatNumber(after-before)}% resist per +${n} ${label}`);
    }else{
      lines.push(`+${n} ${label} reduces Magic damage taken`);
    }
    return lines;
  }
  if(statKey==='spd'){
    const agiPer=(Avian?.data?.combatConfig?.weaponFirst?.agilityDodgePctPerPoint != null)
      ? Number(Avian.data.combatConfig.weaponFirst.agilityDodgePctPerPoint) : 0.5;
    lines.push(`+${formatCombatNumber(n*agiPer)}% Evasion`);
    lines.push(`Derived dodge from Agility`);
    return lines;
  }
  if(statKey==='dodge'){
    const cap=50;
    const next=Math.min(cap, cur+n);
    lines.push(`+${n}% Evasion (${cur}% → ${next}%, cap ${cap}%)`);
    return lines;
  }
  lines.push(`+${n} ${label}`);
  return lines;
...
function getLevelUpCombatImpactLine(statKey, amount){
  const lines=getLevelUpPerPointBreakdown(statKey, 1);
  return lines[0]||'';
}

function getLevelUpGlossaryBlurb(statKey){
  const concepts=Avian?.data?.displayGlossary?.concepts||{};
  if(statKey==='def' && concepts.Guard) return concepts.Guard;
  if(statKey==='mdef' && concepts.Resolve) return concepts.Resolve;
  if(statKey==='dodge' && concepts.Evasion) return concepts.Evasion;
  if(statKey==='acc' && concepts.Precision) return concepts.Precision;
  return '';
}

function luStatKeyForOption(opt){
  if(!opt) return '';
  if(opt.stat==='vitality') return 'vitality';
  if(opt.stat==='dodge') return 'dodge';
  return opt.stat;
}

function getLuFeatherStatValue(stats, statKey){
  if(!stats) return 0;
  if(statKey==='energyMax') return Number(stats.energyMax)||0;
  return Number(stats[statKey])||0;
}

function buildLevelUpStatTooltipHtml(statKey, opt=null){
  const before=G._luStatBaseline||captureLuStatBaseline();
  const after=simulateLuDraftStats();
  const label=(ledgerStatLabel(statKey)||statKey).toUpperCase();
  const cur=getLuFeatherStatValue(before, statKey);
  const proj=getLuFeatherStatValue(after, statKey);
  const isPct=statKey==='dodge';
  const fmt=(v)=>isPct?`${v}%`:String(v);
  let html=`<div class="tt-name">${escapeHtmlRoster(label)}</div>`;
  html+=`<div class="tt-row"><span class="tt-lbl">Current</span><span class="tt-val">${escapeHtmlRoster(fmt(cur))}</span></div>`;
  if(Math.abs(proj-cur)>0.0001){
    html+=`<div class="tt-row"><span class="tt-lbl">Projected</span><span class="tt-val">${escapeHtmlRoster(fmt(proj))}</span></div>`;
    html+=`<div class="tt-row"><span class="tt-lbl">Draft gain</span><span class="tt-val">+${escapeHtmlRoster(fmt(proj-cur))}</span></div>`;
  }
  const perPointLines=getLevelUpPerPointBreakdown(statKey, 1);
  if(perPointLines.length){
    html+=`<div class="tt-divider"></div><div class="tt-type">Per +1 ${escapeHtmlRoster(ledgerStatLabel(statKey)||statKey)}</div>`;
    for(const line of perPointLines){
      html+=`<div class="tt-row"><span class="tt-val" style="font-size:.82em">${escapeHtmlRoster(line)}</span></div>`;
    }
  }
  if(opt){
    const perFeather=Math.max(0, Number(opt.amount)||0);
    if(perFeather>1 || (perFeather===1 && statKey!=='vitality')){
      const featherLines=getLevelUpPerPointBreakdown(opt.stat, perFeather);
      html+=`<div class="tt-divider"></div><div class="tt-type">Per Feather (+${perFeather})</div>`;
      for(const line of featherLines){
        html+=`<div class="tt-row"><span class="tt-val" style="font-size:.82em">${escapeHtmlRoster(line)}</span></div>`;
      }
    }
    if(String(opt.id||'').includes('_rare')||String(opt.id||'').endsWith('r')){
      html+=`<div class="tt-row"><span class="tt-val" style="font-size:.88em;color:var(--purple-light)">Rare endless bonus option</span></div>`;
    }
  }
  const ledgerKey=statKey==='maxHp'?'maxHp':statKey;
  const fromLevel=Number(G.player?._statLedger?.fromLevel?.[ledgerKey])||0;
  if(Math.abs(fromLevel)>0.0001){
    html+=`<div class="tt-row"><span class="tt-lbl">From Level-ups</span><span class="tt-val">+${formatCombatNumber(fromLevel)}</span></div>`;
  }
  const blurb=getLevelUpGlossaryBlurb(statKey);
  if(blurb) html+=`<div class="tt-desc">${escapeHtmlRoster(blurb)}</div>`;
  html+=richTooltipCloseBtn();
  return html;
}

function wireLevelUpTooltips(){
  const opts=G._luFeatherPanelOptions;
  if(Array.isArray(opts)){
    for(const opt of opts){
      const row=document.getElementById(`lu-fr-${opt.id}`);
      if(!row || typeof bindRichTooltip!=='function') continue;
      row._richTooltipBound=false;
      const statKey=luStatKeyForOption(opt);
      bindRichTooltip(row.querySelector('.lu-feather-info')||row, ()=>buildLevelUpStatTooltipHtml(statKey, opt));
    }
  }
  const prevWrap=document.getElementById('lu-stat-preview');
  if(prevWrap && typeof bindRichTooltip==='function'){
    prevWrap.querySelectorAll('.lu-stat-row[data-stat]').forEach(el=>{
      el._richTooltipBound=false;
      const key=el.dataset.stat;
      bindRichTooltip(el, ()=>{
        if(typeof buildRichStatTooltipHtml==='function' && G.player){
          const after=simulateLuDraftStats();
          const raw=Number(after[key])||0;
          return buildRichStatTooltipHtml(key, raw, G.player);
        }
        return buildLevelUpStatTooltipHtml(key);
      });
    });
  }
}

function resetLuFeatherDraft(){
  if(!(G._pendingLevelUpChoices>0)) return;
  G._luFeatherDraft={};
  refreshLuFeatherPanelUI();
}
globalThis.resetLuFeatherDraft=resetLuFeatherDraft;
...
function showLevelUpScreen() {
  ...
  const badge=document.getElementById('lu-level-badge');
  if(badge){
    badge.textContent=`Lv. ${G.player.birdLevel||1}`;
    badge.classList.remove('lu-badge-pulse-replay');
    void badge.offsetWidth;
    badge.classList.add('lu-badge-pulse-replay');
  }
  document.getElementById('lu-sub').textContent=`Lv.${G.player.birdLevel} reached! ${feathers} Feather${feathers===1?'':'s'} to spend — hover stats for damage/health/resist breakdown. Hold +/− to allocate.`;
  renderLuStatPreview();
...
function grantGroveGearReward(outcomeType){
  const stage=G.stage||1;
  if(!Avian.equipmentLoot || typeof Avian.equipmentLoot.rollEquipmentReward!=='function') return null;
  return Avian.equipmentLoot.rollEquipmentReward({ stage, groveOutcome: outcomeType, filterForPlayer: true });
}

function applyGroveGearReward(rw){
  if(!rw) return null;
...
function showGroveRewardCard(rw){
  ...
  if(heading){
    heading.textContent='⚙ Equipment find';
...
function grantGroveNestReward(){
  return applyGroveGearReward(grantGroveGearReward('nest'));
}
...
async function resolveGrove(idx){
  ...
  switch(type){
    ...
    case 'egg':{
      ...
      const rw=applyGroveGearReward(grantGroveGearReward('egg'));
      if(rw){
    ...
    case 'goldenGoose':{
      ...
      const rw=applyGroveGearReward(grantGroveGearReward('goldenGoose'));
      if(added&&rw){
...
function enterStorkShopScreen(){
  ...
  if(log){
    ...
    log.textContent=mode==='endless-boss'
      ? 'Heals and one equipment offer this visit.'
      : 'Buy heals and equipment, sell bag gear, or review what you are wearing.';
  }
  generateShopItems();
  syncShopDock();
}
...
function shopMutationTierKey(item){
  if(!item) return null;
  if(item.type==='equipment'||item.shopCategory==='equipment') return normalizeRewardTier(item.tier);
...
function resolveShopItemCategory(item){
  ...
  if(item.type==='equipment'||item.shopCategory==='equipment') return 'mutations';
...
function getShopCategoryLogText(category){
  const mode=G._shopMode||'boss';
  if(category==='all') return 'Tap cards to multi-select, then buy in one go. Hover or hold a card to compare with worn gear.';
  if(category==='items') return 'Stackable battle heals — buy until your hold cap, then use in combat (1 heal per turn).';
  if(category==='mutations') return mode==='endless-boss'
    ? `One ${shopGearCategoryTitle().toLowerCase()} offer this visit. Hover or hold to compare with worn gear.`
    : `${shopGearCategoryTitle()} — 6 offers per unlocked tier. Hover or hold a card to compare with worn gear.`;
  if(category==='sell') return Avian.flags?.equipmentV2
    ? 'Multi-select bag equipment to sell for half its shop buy price.'
    : 'Multi-select bag items to sell for half their shop buy price.';
  if(category==='equipped') return 'Your worn loadout. Tap a filled slot to unequip it into the bag (then sell from the Sell tab if you want).';
  return '';
}
...
function getShopSelectedSellTotal(){
  const inv=G.player?.equipmentInventory||[];
  let total=0;
  SHOP_STATE.selectedSellIndices.forEach(idx=>{
...
function syncShopDock(){
  const tab=G._shopTab||'buy';
  const category=G._shopCategoryTab||'all';
  const summary=document.getElementById('shop-dock-summary');
  ...
  if(tab==='sell'){
    ...
    if(sellBtn){
      ...
      sellBtn.textContent=n?`Sell selected · +${total}🌟`:'Sell selected';
    }
    return;
  }
  if(tab==='equipped'){
    if(summary) summary.textContent='Tap worn gear to unequip it into the bag.';
    return;
  }
  const n=SHOP_STATE.selectedBuyIndices.size;
  const total=getShopSelectedBuyTotal();
  if(summary) summary.textContent=n?`${n} selected · ${total}🌟`:(category==='all'?'Select offerings to buy.':'Select items to buy.');
  if(buyBtn){
    buyBtn.disabled=n===0;
    buyBtn.textContent=n?`Buy selected · ${total}🌟`:'Buy selected';
  }
}
function updateShopBuyButtonState(){
  ...
  buyBtn.disabled=n===0;
  const total=getShopSelectedBuyTotal();
  buyBtn.textContent=n>0?`Buy selected · ${total}🌟`:'Buy selected';
  syncShopDock();
}
...
function buildShopBuyCard(item, idx, {isSelected, canSelect, cost}){
  ...
  if(item.id) div.dataset.shopItemId=String(item.id);
  if(item.type==='equipment' || item.equipmentItemId){
    div.dataset.shopEqItem=String(item.equipmentItemId||item.id);
  }
  div.innerHTML=`
    ...
    <div class="reward-desc mut-stat-compact-wrap">${item.type==='mutation'||item.mutationItemId?(getMutationDescHtml(item.mutationItemId||item.id,{compact:true})||escapeHtmlRoster(item.desc||'')):item.type==='equipment'?escapeHtmlRoster(item.desc||''):escapeHtmlRoster(item.desc||'')}</div>`;
  if(item.desc && tooltipsEnabled('items') && item.type!=='equipment' && !item.equipmentItemId){
    div.title=item.type==='mutation'?String(item.desc||'').replace(/\n/g,' · '):item.desc;
  }
...
function setShopTab(tab){
  ...
  const isEquipped=(tab==='equipped');
  const leavingBuy=isSell||isEquipped;
  if(leavingBuy){
    SHOP_STATE.selectedBuyIndices.clear();
    SHOP_STATE.selectedIndex=null;
    _shopSelectedIdx=null;
  ...
  SHOP_BUY_CATEGORY_TABS.forEach(cat=>{
    ...
    if(el){
      const on=!leavingBuy && G._shopCategoryTab===cat;
  ...
  if(sellTab){
    ...
    sellTab.setAttribute('aria-selected', String(isSell));
  }
  const eqTab=document.getElementById('shop-tab-equipped');
  if(eqTab){
    eqTab.classList.toggle('active', isEquipped);
    eqTab.setAttribute('aria-selected', String(isEquipped));
  }
  const buyBtn=document.getElementById('shop-buy-btn');
  const sellBtn=document.getElementById('shop-sell-btn');
  const refreshBtn=document.getElementById('shop-refresh-btn');
  if(buyBtn) buyBtn.style.display=leavingBuy?'none':'';
  if(sellBtn) sellBtn.style.display=isSell?'':'none';
  if(refreshBtn) refreshBtn.style.display=leavingBuy?'none':'';
  const grid=document.getElementById('shop-items-grid');
  if(grid) grid.classList.toggle('shop-equipped-grid', isEquipped);
  const log=document.getElementById('shop-purchase-log');
  if(log){
    log.textContent=getShopCategoryLogText(isEquipped?'equipped':(isSell?'sell':G._shopCategoryTab));
  }
  if(isEquipped) renderShopEquipped();
  else if(isSell) renderShopSellItems();
  else renderShopItems();
  syncShopDock();
}

function getEquipmentSellPrice(rarity){
  if(typeof Avian?.equipmentLoot?.getSellPrice==='function') return Avian.equipmentLoot.getSellPrice(rarity);
  const costs=(Avian?.data?.equipment?.loot?.shopCosts)||{grey:15,green:28,blue:44,purple:64,gold:96,orange:200};
  const key=String(rarity||'grey').toLowerCase();
...
function getMutationSellPrice(tier){
  ...
  const costs={white:16,green:28,blue:44,purple:64,gold:96,orange:140};
...
function renderShopSellItems(){
  ...
  if(inv.length){
    ...
    inv.forEach((itemId, idx)=>{
      ...
      <div class="reward-name">${item.name}</div>
      <div class="reward-desc">${escapeHtmlRoster(item.slot||'Equipment')} · Sell for half buy price</div>`;
      div.addEventListener('click', ()=>{
...
function shopSlotIsStarterLocked(player, slotKey, item){
  if(!item || slotKey!=='mainHand') return false;
  return !!(item.starter || item.basic || item.id==='WPN-073' || /starting|basic/i.test(String(item.name||'')));
}
function renderShopEquipped(){
  const grid=document.getElementById('shop-items-grid'); if(!grid) return;
  grid.innerHTML='';
  grid.classList.add('shop-equipped-grid');
  const shinyCt=document.getElementById('shop-shiny-count'); if(shinyCt) shinyCt.textContent=G.shinyObjects;
  if(!G.player || typeof Avian?.equipment?.ensurePlayerEquipmentState!=='function'){
    grid.innerHTML='<div style="grid-column:1/-1;color:var(--text-dim);text-align:center;padding:24px 0;">No loadout to show.</div>';
    syncShopDock();
    return;
  }
  Avian.equipment.ensurePlayerEquipmentState(G.player);
  ...
  order.forEach(slotKey=>{
    ...
    const item=itemId?(Avian.equipment.getItem?.(itemId)||null):null;
    const label=getEquipmentNestSlotLabel(slotKey);
    const icon=EQUIPMENT_NEST_SLOT_ICONS[slotKey]||'⚙';
    const div=document.createElement('div');
    const starterLocked=shopSlotIsStarterLocked(G.player, slotKey, item);
    div.className=`shop-item shop-equipped-slot${item?' is-filled':' is-empty'}${item?` tier-${normalizeRewardTier(item.rarity)}`:''}`;
    div.dataset.shopEquipSlot=slotKey;
    if(itemId) div.dataset.shopEqItem=String(itemId);
    if(item){
      const stats=typeof formatEquipmentCompactStatsHtml==='function'?formatEquipmentCompactStatsHtml(item):'';
      const action=starterLocked
        ? '<span class="shop-equipped-unequip">Starting weapon</span>'
        : '<span class="shop-equipped-unequip">Unequip to bag</span>';
      div.innerHTML=`
        <div class="shop-equipped-slot-label">${icon} ${escapeHtmlRoster(label)}</div>
        <div class="reward-name">${escapeHtmlRoster(item.name)}</div>
        <div class="reward-desc">${escapeHtmlRoster(item.slot||'Equipment')}${stats?` · ${stats.replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim()}`:''}</div>
        ${action}`;
      if(!starterLocked){
        div.addEventListener('click', ()=>shopUnequipSlot(slotKey));
      }
    } else {
      div.innerHTML=`
        <div class="shop-equipped-slot-label">${icon} ${escapeHtmlRoster(label)}</div>
        <div class="reward-name">Empty</div>
        <div class="reward-desc">Nothing worn in this slot.</div>`;
    }
    grid.appendChild(div);
  });
  bindShopItemCompareTooltips();
...
function shopUnequipSlot(slotKey){
  if(!G.player || !slotKey) return false;
  if(typeof Avian?.equipment?.unequip!=='function') return false;
  const before=G.player.equipment?.[slotKey];
  const ok=Avian.equipment.unequip(G.player, slotKey);
  if(!ok){
    const log=document.getElementById('shop-purchase-log');
    if(log) log.textContent='That piece cannot be unequipped here.';
    return false;
  }
  if(typeof reapplyPlayerGearStats==='function') reapplyPlayerGearStats(G.player);
  saveRun();
  const item=before?Avian.equipment.getItem?.(before):null;
  const log=document.getElementById('shop-purchase-log');
  if(log) log.textContent=item?`Unequipped ${item.name} into the bag.`:'Unequipped into the bag.';
  renderShopEquipped();
  return true;
}
globalThis.setShopTab=setShopTab;
globalThis.renderShopEquipped=renderShopEquipped;
globalThis.shopUnequipSlot=shopUnequipSlot;
globalThis.syncShopDock=syncShopDock;
...
function _findShopItemById(id) {
  ...
  if (upg) return upg;
  if (typeof Avian?.equipmentLoot?.reconstructShopOffer === 'function') {
    const eq = Avian.equipmentLoot.reconstructShopOffer(id);
    if (eq) return eq;
  }
...
function renderShopItems() {
  ...
  grid.classList.remove('shop-equipped-grid');
  grid.innerHTML='';
  const shinyCt=document.getElementById('shop-shiny-count'); if(shinyCt) shinyCt.textContent=G.shinyObjects;
  ...
  if(category==='all'){
    ...
    SHOP_CATEGORY_DISPLAY_ORDER.forEach(cat=>{
      ...
      any=true;
      if(cat==='mutations'){
        appendShopSectionHeading(grid, shopGearCategoryTitle());
...
function shopRefresh() {
  ...
  const log=document.getElementById('shop-purchase-log');
  if(log) log.textContent='🔄 Shop refreshed — new equipment stock rolled.';
  generateShopItems();
  return true;
}
...
const DEV_CODE_CATALOG=Object.freeze([
  ...
  {code:'birdwatching',description:'Unlock every playable bird.',reversible:true},
  {code:'headinghome',description:'Lock unlockable birds again.',oneshot:true},
  {code:'blakiston',description:'Unlock Duke Blakiston.',reversible:true},
  {code:'reference',description:'Unlock every Codex entry.',reversible:false},
  {code:'buildnest',description:'Unlock the Build Nest map forge.',reversible:true},
  {code:'combattest',description:'Unlock Combat Scenario Test.',reversible:true},
  {code:'tiergreen',description:'Set every owned bird card to Green tier.',oneshot:true},
  {code:'tierblue',description:'Set every owned bird card to Blue tier.',oneshot:true},
  {code:'tierpurple',description:'Set every owned bird card to Purple tier.',oneshot:true},
  {code:'tiergold',description:'Set every owned bird card to Gold tier.',oneshot:true},
  {code:'goldengoose',description:'Toggle infinite Golden Goose Eggs.',reversible:true},
  {code:'Featherplucked',description:'Add 900 Species Feathers to every owned bird.',oneshot:true},
]);
...
function isDevCodeEnabled(code){const switches=getDevCodeSwitches();return switches[String(code||'').toLowerCase()]===true;}
function saveDevCodeSwitches(switches){
  try{localStorage.setItem(DEV_CODE_SWITCHES_KEY,JSON.stringify(switches||{}));}catch(_){ }
}
function setDevCodeSwitch(code, enabled){
  const switches=getDevCodeSwitches();
  switches[String(code||'').toLowerCase()]=!!enabled;
  saveDevCodeSwitches(switches);
  return switches;
}
function getDevCodeCatalogRow(code){
  const needle=String(code||'').toLowerCase();
  return DEV_CODE_CATALOG.find(row=>String(row.code||'').toLowerCase()===needle)||null;
}
function applyBirdwatchingUnlock(){
  const u=getUnlocks();
  collectAllBirdUnlockIds().forEach(id=>{ u[id]=true; });
...
function applyHeadingHomeLock(){
  const starters=new Set(getPlayableStarterBirdKeys());
  const u=getUnlocks();
  Object.entries(BIRDS||{}).forEach(([key,bird])=>{
    if(starters.has(key)) return;
    if(bird?.unlockRequires) delete u[bird.unlockRequires];
  });
  localStorage.setItem(UNLOCK_KEY, JSON.stringify(u));
  try{ localStorage.removeItem('blakiston_debug_unlocked'); }catch(_){}
  try{ window.__blakistonDebugUnlocked=false; }catch(_){}
}
function setGoldenGooseInfiniteState(on){
  try{localStorage.setItem('avian_infinite_golden_eggs', on?'1':'0');}catch(_){}
...
function toggleDevCode(spec){
  ...
  setDevCodeSwitch(code, enabled);
  renderSuppliesCodeTools();
}
function refreshAfterDevCode(){
  try{ if(typeof syncBuildNestUnlockUI==='function') syncBuildNestUnlockUI(); }catch(_){}
  try{
    if(typeof Avian?.debug?.syncCombatScenarioTestUnlockUI==='function') Avian.debug.syncCombatScenarioTestUnlockUI();
  }catch(_){}
  try{ if(typeof initSelectionSafe==='function') initSelectionSafe(); }catch(_){}
}
function activateDevCode(code, opts){
  opts=opts||{};
  const silent=!!opts.silent;
  const msg=opts.msgEl||document.getElementById('dev-code-msg');
  const say=(text, color)=>{
    if(silent||!msg) return;
    msg.textContent=text;
    msg.style.color=color||'var(--gold-light)';
    if(!opts.keepMsg) setTimeout(()=>{ if(msg && msg.textContent===text) msg.textContent=''; }, 3200);
  };
  if(code==='birdwatching'){
    applyBirdwatchingUnlock();
    say('🔓 Birdwatching: all birds unlocked!');
    if(!opts.skipRefresh) refreshAfterDevCode();
    return true;
  }
  if(/^tier(green|blue|purple|gold)$/.test(code)){
    const tier=code.slice(4); const count=setOwnedBirdTier(tier);
    say(`⬆ ${count} unlocked bird cards upgraded to ${tier}.`);
    if(!opts.skipRefresh) refreshAfterDevCode();
    return true;
  }
  if(code==='featherplucked'){
    ...
    say(`🪶 +900 feathers for ${keys.length} unlocked birds.`);
    return true;
  }
  if(code==='headinghome'){
    applyHeadingHomeLock();
    say('🔒 Headinghome: unlockable birds locked again.','var(--red-light)');
    if(!opts.skipRefresh) refreshAfterDevCode();
    return true;
  }
  if(code==='blakiston'){
    ...
    try{ window.__blakistonDebugUnlocked=true; }catch(_){}
    say('🦉 Duke Blakiston unlocked as a playable bird.');
    if(!opts.skipRefresh) refreshAfterDevCode();
    return true;
  }
  if(code==='reference'){
    unlockAllCodexEntries();
    say('📖 Reference: all Codex entries unlocked for browsing.');
    try{ if(typeof renderReferenceGuide==='function') renderReferenceGuide(); }catch(_){}
    return true;
  }
  if(code==='buildnest'){
    try{ localStorage.setItem('avian_buildnest_unlocked','1'); }catch(_){}
    say('🪺 Build Nest unlocked — map forge available in the war room.');
    if(!opts.skipRefresh) refreshAfterDevCode();
    return true;
  }
  if(code==='combattest'){
    ...
    else { try{ document.body.classList.add('combat-scenario-test-unlocked'); }catch(_){} }
    say('⚔ Combat Scenario Test unlocked — harness available on the left of the war room.');
    if(!opts.skipRefresh) refreshAfterDevCode();
    return true;
  }
  if(code==='goldengoose'){
    const currentlyOn=typeof isGoldenGooseInfinite==='function'&&isGoldenGooseInfinite();
    const infinite=opts.forceState===true?true:opts.forceState===false?false:!currentlyOn;
    setGoldenGooseInfiniteState(infinite);
    say(infinite?'🪿 GoldenGoose: infinite Golden Goose Eggs enabled!':'🪿 Infinite Golden Goose Eggs disabled.');
    return true;
  }
  return false;
}
function deactivateDevCode(code, opts){
  opts=opts||{};
  if(code==='birdwatching' || code==='headinghome'){
    applyHeadingHomeLock();
    if(!opts.skipRefresh) refreshAfterDevCode();
    return true;
  }
  if(code==='blakiston'){
    const u=getUnlocks();
    delete u.unlock_duke_blakiston;
    localStorage.setItem(UNLOCK_KEY, JSON.stringify(u));
    try{ localStorage.removeItem('blakiston_debug_unlocked'); }catch(_){}
    try{ window.__blakistonDebugUnlocked=false; }catch(_){}
    if(!opts.skipRefresh) refreshAfterDevCode();
  ...
  if(code==='buildnest'){
    try{ localStorage.removeItem('avian_buildnest_unlocked'); }catch(_){}
    if(!opts.skipRefresh) refreshAfterDevCode();
    return true;
  }
  if(code==='combattest'){
    try{ localStorage.removeItem('avian_combattest_unlocked'); }catch(_){}
    try{ document.body.classList.remove('combat-scenario-test-unlocked'); }catch(_){}
    if(typeof Avian?.debug?.syncCombatScenarioTestUnlockUI==='function') Avian.debug.syncCombatScenarioTestUnlockUI();
    if(!opts.skipRefresh) refreshAfterDevCode();
    return true;
  }
  if(code==='goldengoose'){
    setGoldenGooseInfiniteState(false);
    return true;
  }
  return false;
}
function applyDevCodeSwitches(){
  const prev=getDevCodeSwitches();
  const switches={};
...
function renderSuppliesCodeTools(){
  ...
  const subnav=document.querySelector('#select-hub-supplies .supplies-subnav');
  if(subnav) subnav.classList.toggle('hub-subnav--3', !!creator);
...
function isKnownDevCodePrefix(val){
  const needle=String(val||'').trim().toLowerCase();
  if(!needle) return false;
  return DEV_CODE_CATALOG.some(row=>String(row.code||'').toLowerCase().startsWith(needle));
}
function checkDevCode(val) {
  ...
  if(code==='gmbirdwatching'){
    try{localStorage.setItem(CREATOR_CODES_KEY,'1');}catch(_){}
    renderSuppliesCodeTools();
    try{ if(typeof setSuppliesSubView==='function') setSuppliesSubView('codes'); }catch(_){}
    const input=document.getElementById('dev-code-input');
    if(input) input.value='';
    if(msg){msg.textContent='🛠 Creator code switches ready — tick a code and press Apply.';msg.style.color='var(--gold-light)';}
    return;
  }
  ...
  if(val.length>=10 && !isKnownDevCodePrefix(code)){
...
globalThis.getPlayableStarterBirdKeys=getPlayableStarterBirdKeys;
globalThis.collectAllBirdUnlockIds=collectAllBirdUnlockIds;
globalThis.isBirdUnlockedForSelect=isBirdUnlockedForSelect;
globalThis.applyBirdwatchingUnlock=applyBirdwatchingUnlock;
globalThis.applyHeadingHomeLock=applyHeadingHomeLock;
...
const MUSIC_PREVIEW_TITLES={
  menu:'Blakiston Theme',
  sharp_beak:'Sharp Beak Quick Wing',
  brittle_waltz:'The Brittle Waltz',
  pigeon_jig:"The Pigeon's Desperate Jig",
  last_thermal:'The Last Thermal',
  duke:'Duke Blakiston Battle',
};
function resolvedMusicTrackIdForRole(role, overrides){
  const choice=overrides?.[role]||'default';
  if(choice && choice!=='default') return choice;
  if(role==='menu') return 'menu';
  if(role==='overworld') return 'last_thermal';
  return null;
}
function syncMusicMenuNowPlaying(trackId){
  const el=document.getElementById('music-menu-now-playing');
  if(!el) return;
  if(!trackId){
    el.textContent='Preview stopped — menu music will resume.';
    el.classList.remove('is-live');
    return;
  }
  el.textContent='Previewing '+ (MUSIC_PREVIEW_TITLES[trackId]||trackId);
  el.classList.add('is-live');
}
function syncMusicMenuAssignmentChips(){
  const api=getBgmApi();
  const overrides=api?.getMusicOverrides?api.getMusicOverrides():{menu:'default',battle:'default',overworld:'default'};
  const roleLabels={menu:'Menu',battle:'Battle',overworld:'Overworld'};
  const byTrack={};
  ['menu','battle','overworld'].forEach(role=>{
    const tid=resolvedMusicTrackIdForRole(role, overrides);
    if(!tid) return;
    (byTrack[tid]=byTrack[tid]||[]).push(roleLabels[role]);
  });
  document.querySelectorAll('.music-menu-item[data-track-id]').forEach(item=>{
    const rolesEl=item.querySelector('.music-menu-track-roles');
    const roles=byTrack[item.dataset.trackId]||[];
    if(rolesEl){
      rolesEl.textContent=roles.join(' · ');
      rolesEl.hidden=!roles.length;
    }
    item.classList.toggle('is-assigned', roles.length>0);
  });
}
function syncMusicMenuControls(){
  ...
  syncMusicMenuAssignmentChips();
  if(!isMusicMenuPreviewActive()) syncMusicMenuNowPlaying(null);
}
...
function playMusicMenuPreview(trackId){
  ...
  syncMusicMenuNowPlaying(trackId);
}
function stopMusicMenuPreview(){
  ...
  if(api) api.fadeOut(el,{durationMs:800,onDone:()=>{
    ...
    document.querySelectorAll('.music-menu-item.is-playing').forEach(item=>item.classList.remove('is-playing'));
    syncMusicMenuNowPlaying(null);
    tryPlayThemeBgmForCurrentMenuScreen();
  }});
  else{
...
function normalizeAccessibilitySettings(raw){
  ...
  return {
    ...
    showEnemyIntent:raw.showEnemyIntent!==false,
    audio,
...
function selectSettingsTab(ev){
  ...
  document.querySelectorAll('.settings-tab').forEach(t=>{
    const on=t===btn || t.dataset.settingsTab===tab;
...
function syncAudioSettingLabels(){
  const pairs=[
    ['setting-master-volume','setting-master-volume-val'],
    ['setting-music-volume','setting-music-volume-val'],
    ['setting-sfx-volume','setting-sfx-volume-val'],
  ];
  pairs.forEach(([id,vid])=>{
    const input=document.getElementById(id);
    const lab=document.getElementById(vid);
    if(input&&lab) lab.textContent=`${Math.round(Number(input.value)||0)}%`;
  });
}
...
function applyAccessibilitySettings(s){
  ...
  if(document.getElementById('screen-battle')?.classList.contains('active') && typeof renderEnemyPlan==='function'){
    try{ renderEnemyPlan(); }catch(_){}
  }
...
function openSettingsModal(){
  ...
  const showIntent=document.getElementById('setting-show-enemy-intent');
  if(showIntent) showIntent.checked=cfg.showEnemyIntent!==false;
  const ms=getMusicSettings();
  const a=cfg.audio||{};
...
function updateAccessibilitySettings(){
  ...
  const cfg={
    ...
    showEnemyIntent:document.getElementById('setting-show-enemy-intent')
      ? !!document.getElementById('setting-show-enemy-intent').checked
      : prev.showEnemyIntent!==false,
    ...
    globalThis.executeEnemyAction = async function(act){
      try{
        if(act?.type === 'attack' || act?.type === 'strike' || act?.type === 'heavy' || act?.abilityId === 'eStun'){
          if(!combatFxOwnsAnim()) playAttackMotion('enemy');
        }
      }catch(_){}
      return await oldExec.apply(this, arguments);
    };
