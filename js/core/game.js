...
function openNest() {
  ...
  <div class="nest-stats-grid">
    ...
    <div class="nest-stat-card" title="Armour — absorbs martial damage before Health"><div class="nest-stat-val" style="color:#b8c4d0">${formatCombatNumber(Math.max(0,Number(s.armour??s.maxArmour??s.armourFlat??0)||0))}/${formatCombatNumber(Math.max(0,Number(s.maxArmour??s.normalMaxArmour??s.armourFlat??0)||0))}</div><div class="nest-stat-lbl" style="color:#9aa8b8">${ledgerStatLabel('armour',{short:true})}</div></div>
    <div class="nest-stat-card" title="Magic Armour — absorbs magical damage before Health"><div class="nest-stat-val" style="color:#5ecfc0">${formatCombatNumber(Math.max(0,Number(s.magicArmour??s.maxMagicArmour??s.magicArmourFlat??0)||0))}/${formatCombatNumber(Math.max(0,Number(s.maxMagicArmour??s.normalMaxMagicArmour??s.magicArmourFlat??0)||0))}</div><div class="nest-stat-lbl" style="color:#4ab8a8">${ledgerStatLabel('magicArmour',{short:true})}</div></div>
...
function buildEnemyInfoPopupMutationsHtml(enemy){
  ...
  const slots=Avian.data?.equipment?.slots?.slotOrder||[];
  const rows=slots.map(sk=>{
    const id=enemy.equipment[sk];
    if(!id) return '';
    const item=Avian?.equipment?.getItem?.(id);
    if(!item) return '';
    return `<div class="enemy-info-mut-row"><span class="enemy-info-mut-icon">⚙</span><div class="enemy-info-mut-body"><div class="enemy-info-mut-name">${escapeHtmlRoster(item.name)}</div></div></div>`;
  }).filter(Boolean);
