#!/usr/bin/env node
/* Verify reward/shop UI modules expose required globals (Step 7 Phase 5). */
import { readFileSync, existsSync } from 'node:fs';

const files = [
  'js/ui/reward-screen.js',
  'js/ui/shop-compare.js',
  'js/systems/shop-cadence.js',
];

const globals = [
  'showRewardScreen',
  'confirmReward',
  'applySingleReward',
  'handleNestShake',
  'buildShopCompareTooltipHtml',
  'bindShopItemCompareTooltips',
  'isShopDueAfterBattle',
  'isGreyShopStage',
];

const checks = [];
for (const f of files) {
  checks.push({ name: `exists ${f}`, ok: existsSync(f) });
}
const gameJs = readFileSync('js/core/game.js', 'utf8');
for (const g of ['showRewardScreen', 'confirmReward', 'bindShopItemCompareTooltips', 'isShopDueAfterBattle']) {
  checks.push({ name: `game.js no longer defines ${g}`, ok: !new RegExp(`function ${g}\\(`).test(gameJs) });
}
const bundle = existsSync('js/avian-game.bundle.js') ? readFileSync('js/avian-game.bundle.js', 'utf8') : '';
for (const g of globals) {
  const ok = bundle.includes(`globalThis.${g}`) || bundle.includes(`function ${g}(`);
  checks.push({ name: `global ${g}`, ok });
}
const failed = checks.filter((c) => !c.ok);
if (failed.length) {
  console.error('verify-reward-shop FAILED:');
  for (const f of failed) console.error(' -', f.name);
  process.exit(1);
}
console.log(`verify-reward-shop: ${checks.length} checks passed`);
