...
const game = readFileSync(path.join(ROOT, 'js/core/game.js'), 'utf8')
  + readFileSync(path.join(ROOT, 'js/ui/shop-compare.js'), 'utf8');
...
const htmlChecks = [
  ...
  ['shop items scroll region', /id="shop-items-scroll"/],
...
const cssChecks = [
  ...
  ['shop scroll region touch scroll', /\.shop-items-scroll\{[\s\S]*?-webkit-overflow-scrolling:touch/,],
...
const jsChecks = [
  ...
  ['multi-select misc sell set', /selectedSellMiscIndices:new Set\(\)/],
  ['misc sell section', /appendShopSectionHeading\(grid, 'Miscellaneous'\)/],
  ['misc bonus shines helper', /function addPlayerMiscBonusShines\b/],
