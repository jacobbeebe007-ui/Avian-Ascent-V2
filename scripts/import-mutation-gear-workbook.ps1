...
function Write-MutationDataFile {
    ...
    $lines = @(
        $Header
        '(function(){''use strict'';'
        ('globalThis.Avian||(globalThis.Avian={{}});Avian.data=Avian.data||Object.create(null);Avian.data.mutations=Avian.data.mutations||Object.create(null);Avian.data.mutations.{0}=Object.freeze({1});' -f $VarName, $json)
        '})();'
    )
...
try {
    ...
    foreach ($tier in $script:TIER_KEYS) {
        $n = $byTier[$tier].Count
        $hdr = '/* GENERATED items-' + $tier + ' - ' + $n + ' items */'
        Write-MutationDataFile (Join-Path $outDir ('items-' + $tier + '.js')) $hdr ('items_' + $tier) $byTier[$tier]
    ...
    foreach ($tier in $script:TIER_KEYS) {
        $indexLines += ('if(m.items_{0}){{for(var k in m.items_{0})byId[k]=m.items_{0}[k];}}' -f $tier)
    }
    $dwJson = ConvertTo-JsString $script:DROP_WEIGHTS
    $indexLines += 'm.byId=Object.freeze(byId);'
    $indexLines += ('m.dropWeights=Object.freeze({0});' -f $dwJson)
    $indexLines += ('m.version=''{0}'';' -f $script:MUTATIONS_VERSION)
    $indexLines += '})();'
    [System.IO.File]::WriteAllText((Join-Path $outDir 'index.js'), (($indexLines -join [Environment]::NewLine) + [Environment]::NewLine), [Text.Encoding]::UTF8)
