#!/usr/bin/env node
/*
 * Import Avian Ascent Equipment System from the Current Master workbook into
 * js/data/equipment/* without overwriting v0.9 bird base stats / combat-pack.
 *
 * Source: Avian_Ascent_Current_Master_v1.5_Physical_Ailments.xlsx
...
const DEFAULT_WORKBOOK = path.join(ROOT, 'Avian_Ascent_Current_Master_v1.5_Physical_Ailments.xlsx');
const WORKBOOK = process.env.AA_EQUIPMENT_WORKBOOK || DEFAULT_WORKBOOK;
const PACK_VERSION = '2026.07-equipment-v1.5-physical-ailments';
...
function parseAilmentRoll(effectText) {
  ...
  const guaranteed = text.match(new RegExp(
  ...
  if (guaranteed) {
    ...
    out.ailmentStacks = Number(guaranteed[1]) || 1;
    return out;
  }
  ...
  const rollNamed = text.match(new RegExp(
  ...
  if (rollNamed) {
    ...
    if (rollNamed[2]) out.ailmentStacks = Number(rollNamed[2]) || 1;
  }
  return out;
}
